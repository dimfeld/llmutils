...
export class RmplanHandler extends CommandHandler {
  readonly definition: CommandDefinition = TOOL_COMMANDS.rmplan;
  ...
  async execute(command: EnhancedCommand, context: ExecutionContext): Promise<void> {
    ...
    try {
      ...
      // Execute rmplan
      const result = await spawnAndLogOutput(rmplanArgs, { cwd: tempDir });
