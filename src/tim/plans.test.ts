...
describe('resolvePlanFile', () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'tim-test-')));
...
describe('plan UUID handling', () => {
  it('generates and persists UUIDs for legacy plans without one', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'tim-uuid-'));
    try {
  ...
  it('throws when UUID persistence fails during legacy plan migration', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'tim-uuid-fail-'));
    try {
      const planPath = join(tempDir, 'legacy.yml');
      await writeFile(
        planPath,
...
describe('readAllPlans', () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'tim-readall-test-')));
  ...
  it('should exclude plans with not_tim: true', async () => {
    const notTimDir = await mkdtemp(join(tmpdir(), 'not-tim-test-'));
    try {
      // Create a normal plan
      await writeFile(
        join(notTimDir, 'normal.yml'),
        stringifyWithFrontmatter({
          id: 400,
          title: 'Normal Plan',
          goal: 'Test',
      ...
      // Create a plan marked as not_tim
      await writeFile(
      ...
      const { plans, maxNumericId } = await readAllPlans(notTimDir);
      ...
      await rm(notTimDir, { recursive: true, force: true });
    }
  });
});
...
describe('findNextReadyPlan', () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'tim-findnext-test-')));
...
describe('setPlanStatus', () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'tim-setstatus-test-')));
  });
...
describe('Plan File Reading and Writing', () => {
  ...
  beforeAll(async () => {
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'tim-frontmatter-test-')));
  });
...
describe('fancy quotes normalization', () => {
  let tempDir: string;

  beforeAll(async () => {
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'tim-quotes-test-')));
  });
  ...
  it('should convert fancy quotes to regular quotes in all string fields except details', async () => {
    ...
    const planWithFancyQuotes: PlanSchema = {
      id: 200,
      title: 'Plan with \u201Cfancy\u201D quotes',
      goal: 'Test that \u201Ccurly quotes\u201D are normalized',
      details: `# Details with \u2018fancy\u2019 quotes
...
These should be preserved in the details field.`,
      ...
      tasks: [
        {
          title: 'Task with \u201Cfancy quotes\u201D',
          description: 'This description has \u2018curly quotes\u2019 too',
          files: ['file-with-\u201Cquotes\u201D.ts'],
          done: false,
          steps: [
            { prompt: 'Step with \u201Cfancy\u201D quotes', done: false },
            { prompt: 'Another step with \u2018quotes\u2019', done: false },
          ],
        },
      ],
    };
  ...
  it('should handle plan files with fancy quotes from external sources', async () => {
    const planPath = join(tempDir, 'external-fancy-quotes.md');

    // Simulate a plan that might come from copying text from a word processor
    const externalContent = `---
id: 201
title: "Plan from Word Processor"
goal: This came from a document with \u201Cfancy quotes\u201D
status: pending
tasks: []
---
...
The system should handle this gracefully and preserve them in the details field.`;
  ...
  it('should preserve regular quotes and not affect other characters', async () => {
    const planPath = join(tempDir, 'regular-quotes.md');
    const planWithRegularQuotes: PlanSchema = {
      id: 202,
      title: 'Plan with "regular" quotes',
      goal: 'Test that regular quotes are not affected',
      details: `# Normal Content

This has regular "double quotes" and 'single quotes'.
It also has other characters like: @ # $ % ^ & * ( ) - + = { } [ ] | \\ : ; < > , . ? /

All of these should remain unchanged.`,
      status: 'pending',
      tasks: [],
    };

    await writePlanFile(planPath, planWithRegularQuotes);

    const fileContent = await readFile(planPath, 'utf-8');
    const readBack = await readPlanFile(planPath);

    // All special characters should be preserved
    expect(readBack.details).toContain('@ # $ % ^ & * ( ) - + = { } [ ] | \\ : ; < > , . ? /');
    expect(readBack.title).toBe('Plan with "regular" quotes');
  });
});
