...
describe('runProofGeneration', () => {
  ...
  const makeConfig = (instructionsOverride?: string | null, artifactsDirOverride?: string) => ({
    ...getDefaultConfig(),
    proofGeneration: {
      instructions:
        instructionsOverride === null ? undefined : (instructionsOverride ?? PROOF_INSTRUCTIONS),
      artifactsDir: artifactsDirOverride ?? '.tim/proofs',
    },
  });
  ...
  test('uses custom artifactsDir from config', async () => {
    const customArtifactsDir = '.tim/custom-proofs';
    const config = makeConfig(undefined, customArtifactsDir);
    const expectedAbsoluteDir = path.resolve(tempDir, customArtifactsDir);

    const { executor, getPrompt } = makeFakeExecutor({
      getTargetDir: () => expectedAbsoluteDir,
    });
    vi.mocked(buildExecutorAndLog).mockReturnValue(executor);

    const result = await runProofGeneration({ ...makeOptions(), config });

    expect(getPrompt()).toContain(expectedAbsoluteDir);
    expect(result.attachedArtifactUuids).toHaveLength(2);
  });
  ...
  // ─── artifactsDir path validation ────────────────────────────────────────

  test('rejects absolute artifactsDir', async () => {
    const config = makeConfig(undefined, '/tmp/absolute-proof-path');
    await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
      /workspace-relative/i
    );
  });

  test('rejects artifactsDir that escapes the workspace via ..', async () => {
    const config = makeConfig(undefined, '../escapes');
    await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
      /inside the workspace/i
    );
  });

  test('rejects artifactsDir that resolves to the workspace root itself', async () => {
  ...
  test('rejects artifactsDir that is a symlink pointing outside the workspace', async () => {
  ...
  test('rejects artifactsDir that is a symlink pointing inside the workspace', async () => {
  ...
  test('rejects artifactsDir that is .git (reserved metadata)', async () => {
    const config = makeConfig(undefined, '.git');
    await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
      /under '\.tim\//
    );
  });

  test('rejects artifactsDir that is .tim itself (must be a strict descendant)', async () => {
    const config = makeConfig(undefined, '.tim');
    await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
      /under '\.tim\//
    );
  });

  test('rejects artifactsDir under a source directory', async () => {
    const config = makeConfig(undefined, 'src/proofs');
    await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
      /under '\.tim\//
    );
  });

  test('rejects reserved tim-managed children (.tim/config, .tim/plans, .tim/workspaces, .tim/reviews)', async () => {
    for (const reserved of [
      ...
      const config = makeConfig(undefined, reserved);
      await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
        /reserved tim-managed directory/i
      );
    }
  });

  test('rejects reserved children case-insensitively (case-insensitive filesystems)', async () => {
    for (const reserved of ['.tim/CONFIG', '.tim/Plans', '.tim/WORKSPACES']) {
      const config = makeConfig(undefined, reserved);
      await expect(runProofGeneration({ ...makeOptions(), config })).rejects.toThrow(
        /reserved tim-managed directory/i
      );
