...
describe('workspace_identifier', () => {
  ...
  afterEach(async () => {
    if (originalEnv.TIM_USER === undefined) {
      delete process.env.TIM_USER;
    } else {
      process.env.TIM_USER = originalEnv.TIM_USER;
    }
  ...
  test('getUserIdentity prefers TIM_USER over other environment variables', () => {
    process.env.USER = 'bob';
    process.env.USERNAME = 'charlie';
    process.env.TIM_USER = ' alice ';
