...
import {
  ...
  type SubagentEnvironmentIdentity,
...
export interface CodexPersistentAgentLaunchOptions {
  ...
  readonly agentEnvironmentIdentity?: SubagentEnvironmentIdentity;
...
export function validateCodexPersistentAgentLaunchOptions(
  ...
  if (value.agentEnvironmentIdentity !== undefined) {
    const environmentIdentity = agentEnvironmentIdentitySchema.parse(
      value.agentEnvironmentIdentity
    );
    if (
      environmentIdentity.role !== 'subagent' ||
      environmentIdentity.id !== identity.id ||
      environmentIdentity.name !== identity.name ||
      environmentIdentity.type !== identity.type
    ) {
      throw new TypeError(
        'Codex persistent-agent environment identity must match its agent identity'
      );
    }
  }
