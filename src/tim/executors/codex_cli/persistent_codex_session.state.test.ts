...
async function createHarness(
  ...
  const tunnelServerReady = createDeferred<{
...
describe('persistent Codex turn state machine', () => {
  ...
  it('steers an active turn only after Codex accepts the expected turn', async () => {
    ...
    expect(harness.connection.turnSteer).toHaveBeenCalledWith({
      ...
      input: [
        {
          type: 'text',
          text: 'Agent message from state-test-agent [id: agent-state-test]:\nCheck the migration.',
        },
      ],
  ...
  it('ignores a late turn/started notification when no turn is active', async () => {
    const harness = await createHarness();
    completeTurn(harness, 'turn-1');
    ...
    expect(harness.handle.providerState).toBe('running-idle');
    expect(harness.handle.input.activity).toBe('idle');
  ...
  it('records FinishTimAgent close intent and closes after assistant output and turn completion', async () => {
    const harness = await createHarness();
  ...
  it('marks the logical thread failed when graceful cleanup reports an error', async () => {
    ...
    expect(harness.logicalLifecycle.markExited).toHaveBeenCalledWith({
      exitCode: 1,
    });
  });
  ...
  it('rejects an unrelated early thread/started notification', async () => {
    ...
    await expect(harness.handle.completion).resolves.toMatchObject({
      error: expect.objectContaining({
        message: expect.stringMatching(/conflicting thread IDs/i),
      }),
    });
  ...
  it('closes a tunnel created after forced startup close and removes its temp directory', async () => {
    ...
    await expect(fs.stat(path.dirname(socketPath))).rejects.toMatchObject({
      code: 'ENOENT',
    });
  });
});
