...
export class PersistentCodexSessionRuntime {
  ...
  private connectionCreationPromise: Promise<CodexAppServerConnection | undefined> | undefined;
  ...
  public async initialize(callbacks: PersistentCodexRuntimeCallbacks): Promise<void> {
    ...
    const connectionCreation = CodexAppServerConnection.create({
      ...
      agentEnvironmentIdentity: this.options.agentEnvironmentIdentity,
    ...
    }).then(async (createdConnection): Promise<CodexAppServerConnection | undefined> => {
      if (!this.isClosing) return createdConnection;
      await createdConnection.close().catch((error: unknown) => {
        debugLog('Persistent Codex late connection cleanup failed:', error);
      });
      return undefined;
    });
    this.connectionCreationPromise = connectionCreation;
    let connection: CodexAppServerConnection | undefined;
    try {
      connection = await connectionCreation;
    } finally {
      if (this.connectionCreationPromise === connectionCreation) {
        this.connectionCreationPromise = undefined;
      }
    }
  ...
  private async finishClose(
    ...
    const connectionCreation = this.connectionCreationPromise;
    if (connectionCreation !== undefined) {
      await connectionCreation.catch((error: unknown) => {
        cleanupError ??= toError(error);
        debugLog('Persistent Codex connection creation cleanup failed:', error);
      });
    }
