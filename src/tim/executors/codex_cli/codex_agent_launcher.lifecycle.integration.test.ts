...
async function createHarness(options: {
  ...
  const launcher = createCodexAgentLauncher({
    ...
    messagingDirectory: '/tmp/tim-agent-lifecycle-session',
  });
  let nextId = 0;
...
describe('Codex AgentManager lifecycle integration', () => {
  it('waits for an app-server that finishes starting during manager shutdown', async () => {
    const connection = createConnection('thread-late-start', 'process-late-start');
    const connectionReady = createDeferred<TestConnection>();
    mocked.create.mockImplementationOnce(async (options): Promise<TestConnection> => {
      const createdConnection = await connectionReady.promise;
      createdConnection.onNotification = options.onNotification;
      createdConnection.onServerRequest = options.onServerRequest;
      createdConnection.onExit = options.onExit;
      mocked.connections.push(createdConnection);
      return createdConnection;
    });
    const { manager, root } = await createHarness({ connections: [] });

    const start = manager.startAgent(root, {
      name: 'late-start-worker',
      type: 'implementer',
      executor: 'codex-cli',
      initialMessage: 'Start after shutdown begins.',
    });
    const startResult = start.catch(() => undefined);
