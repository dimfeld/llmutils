...
import { createAgentEnvironmentIdentity } from '../../agent_messaging/environment.js';
...
export interface ClaudeAgentLauncherOptions {
  ...
  /** Private agent-messaging runtime directory shared by this session. */
  readonly messagingDirectory: string;
}
  ...
  return {
    launch: async (request: AgentLaunchRequest): Promise<AgentLaunchHandle> => {
      ...
      return await runClaudeSubprocess({
        ...
        agentEnvironmentIdentity: createAgentEnvironmentIdentity(
          options.messagingDirectory,
          request.identity
        ),
        claudeCodeOptions: {
          ...configuredOptions,
