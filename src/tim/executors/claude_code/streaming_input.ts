...
/** Resolve only after the subprocess input sink accepts the message. */
export async function sendFollowUpMessageAndWait(stdin: FileSink, content: string): Promise<void> {
  await stdin.write(buildSingleUserInputMessageLine(content));
}
...
export class PersistentClaudeInputWriter {
  ...
  /** Allow writes while launch readiness is still waiting for the initial write. */
  public markWritable(): void {
    if (this.stdinEnded) return;
    this.readyState = true;
    this.notifyAvailabilityChange();
  }

  /** Mark the subprocess input as ready after its initial write is accepted. */
