...
export class PersistentClaudeSessionRuntime {
  ...
  private terminateProvider: (() => Promise<void>) | undefined;
  ...
  public attach(
    ...
    terminateProvider: () => Promise<void>
  ): void {
    ...
    this.terminateProvider = terminateProvider;
  }
  ...
  public createHandle(): ClaudePersistentAgentLaunchHandle {
    ...
    return {
      ...
      release: (): Promise<void> => this.release(),
    };
  }
  ...
  private release(): Promise<void> {
    ...
    this.releasePromise = (async (): Promise<void> => {
      this.forcedShutdownAccepted = true;
      this.inputController?.cleanup();
      if (!this.completionFinalized) {
        const terminateProvider = this.terminateProvider;
        if (terminateProvider === undefined) {
          throw new Error(`Claude ${this.options.label} persistent runtime cannot terminate`);
        }
