...
If during testing you discover that the plan needs to change (e.g. expected behavior differs from the plan, test requirements need updating, or new edge cases emerge that weren't anticipated), report these findings clearly in your output so the orchestrator can update the plan.
...
IMPORTANT: When you determine that the work is acceptable and all verification checks pass, you MUST mark the completed tasks as done using the tim set-task-done command. Use the task titles from the plan to identify which tasks to mark complete.
