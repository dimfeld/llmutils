...
import { formatLogData } from '../../../logging/common.js';
...
function writeLog(message: string, ...values: unknown[]): void {
  ...
  const line = formatLogData(`${[message, ...renderedValues].join(' ')}\n`);
  try {
...
export function createClaudeMcpServer(options: ClaudeMcpServerOptions = {}): ClaudeMcpServerHandle {
  ...
  if (requestedToolNames.has('StartTimAgent')) {
    addAgentTool(server, toolNames, toolDescriptions, toolExecutors, 'StartTimAgent', {
      name: 'StartTimAgent',
      description: AGENT_TOOL_DESCRIPTIONS.StartTimAgent,
      parameters: startAgentArgumentsSchema,
      execute: async (input) =>
        modelVisibleAgentResult(await requestAgentToolFromParent('StartTimAgent', input)),
    });
  }
  if (requestedToolNames.has('ListTimAgents')) {
    addAgentTool(server, toolNames, toolDescriptions, toolExecutors, 'ListTimAgents', {
      name: 'ListTimAgents',
      description: AGENT_TOOL_DESCRIPTIONS.ListTimAgents,
      parameters: listAgentsArgumentsSchema,
      execute: async (input) =>
        modelVisibleAgentResult(await requestAgentToolFromParent('ListTimAgents', input)),
    });
  }
  if (requestedToolNames.has('SendTimAgentMessage')) {
    addAgentTool(server, toolNames, toolDescriptions, toolExecutors, 'SendTimAgentMessage', {
      name: 'SendTimAgentMessage',
      description: AGENT_TOOL_DESCRIPTIONS.SendTimAgentMessage,
      parameters: sendAgentMessageArgumentsSchema,
      execute: async (input) =>
        modelVisibleAgentResult(await requestAgentToolFromParent('SendTimAgentMessage', input)),
    });
  }
  if (requestedToolNames.has('StopTimAgent')) {
    addAgentTool(server, toolNames, toolDescriptions, toolExecutors, 'StopTimAgent', {
      name: 'StopTimAgent',
      description: AGENT_TOOL_DESCRIPTIONS.StopTimAgent,
      parameters: stopAgentArgumentsSchema,
      execute: async (input) =>
        modelVisibleAgentResult(await requestAgentToolFromParent('StopTimAgent', input)),
    });
  }
  if (requestedToolNames.has('FinishTimAgent')) {
    addAgentTool(server, toolNames, toolDescriptions, toolExecutors, 'FinishTimAgent', {
      name: 'FinishTimAgent',
      description: AGENT_TOOL_DESCRIPTIONS.FinishTimAgent,
      parameters: finishAgentArgumentsSchema,
      execute: async (input) =>
        modelVisibleAgentResult(await requestAgentToolFromParent('FinishTimAgent', input)),
    });
  }
