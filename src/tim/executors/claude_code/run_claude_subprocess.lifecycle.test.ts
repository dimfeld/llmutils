...
async function setupPersistentClaudeSubprocess(
  ...
  mockSpawnWithStreamingIO.mockImplementation(async (_args: string[], opts: any) => {
    ...
    const result = new Promise<SpawnAndLogOutputResult>((resolve) => {
      resolveStreamingResult = resolve;
    });
    return {
      pid: 456,
...
describe('runClaudeSubprocess lifecycle', () => {
  ...
  test('an asynchronous initial write failure rejects launch before returning a handle', async () => {
    ...
    const launch = runClaudeSubprocess({
      ...makeSubprocessOptions(),
      mode: 'persistent-agent',
      processLabel: 'Claude agent (persistent-worker)' as AgentProcessLabel,
    });
    await vi.waitFor(() => expect(stdinWriteSpy).toHaveBeenCalledOnce());
    rejectWrite?.(new Error('async stdin boom'));

    await expect(launch).rejects.toThrow('async stdin boom');
    expect(stdinEndSpy).toHaveBeenCalledOnce();
  });

  test('release terminates the provider and remains idempotent after exit', async () => {
    const setup = await setupPersistentClaudeSubprocess();
    const transformEnvironment = setup.spawnOptions.transformEnvironment as (
      env: Record<string, string>
    ) => Record<string, string>;
    expect(
      transformEnvironment({ TIM_AGENT_ID: 'stale-id', TIM_AGENT_TYPE: 'reviewer' })
    ).toMatchObject({
      TIM_AGENT_MESSAGING_DIR: '/tmp/tim-agent-session',
      TIM_AGENT_ID: 'persistent-worker-id',
      TIM_AGENT_NAME: 'persistent-worker',
      TIM_AGENT_TYPE: 'implementer',
      TIM_AGENT_ROLE: 'subagent',
    });

    await expect(setup.handle.release()).resolves.toBeUndefined();
    await expect(setup.handle.release()).resolves.toBeUndefined();
    expect(setup.killSpy).toHaveBeenCalledOnce();
    expect(setup.killSpy).toHaveBeenCalledWith('SIGTERM');
  ...
  test('output produced after release starts is rejected instead of processed', async () => {
    ...
    // release() closes input and marks the runtime synchronously. A process
    // output chunk delivered afterward must not reach the formatter pipeline
    // while provider termination is still pending.
