...
describe('Claude tim MCP child server', () => {
  ...
  test('filters unknown capability-manifest names from child tool installation', () => {
    const result = createClaudeMcpServer({
      ...
      agentToolNames: ['ListTimAgents', 'not-a-tim-tool', 'FinishTimAgent'],
    });

    expect(result.toolNames).toEqual(['ListTimAgents', 'FinishTimAgent']);
    expect([...result.toolExecutors.keys()]).toEqual(['ListTimAgents', 'FinishTimAgent']);
  });
  ...
  test('does not include a caller identity in agent tool requests and renders a correlated result', async () => {
    const result = createClaudeMcpServer({
      interactiveApprovalEnabled: false,
      agentToolNames: ['ListTimAgents'],
    });
    const execute = result.toolExecutors.get('ListTimAgents');
  ...
  test.each([
    {
      toolName: 'StartTimAgent' as const,
      input: { type: 'tester', executor: 'claude-code', initialMessage: 'check this' },
    },
    { toolName: 'ListTimAgents' as const, input: {} },
    {
      toolName: 'SendTimAgentMessage' as const,
      input: { name: 'orchestrator', message: 'status' },
    },
    { toolName: 'StopTimAgent' as const, input: { name: 'worker-a', force: true } },
    { toolName: 'FinishTimAgent' as const, input: { message: 'complete' } },
  ...
  test('does not allow FinishTimAgent to target another agent', () => {
    expect(
      CLAUDE_AGENT_ARGUMENT_SCHEMAS.FinishTimAgent.safeParse({ name: 'worker-a' }).success
    ).toBe(false);
  });
  ...
  test('correlates concurrent calls by request ID even when responses arrive out of order', async () => {
    const result = createClaudeMcpServer({
      interactiveApprovalEnabled: false,
      agentToolNames: ['ListTimAgents', 'SendTimAgentMessage'],
    });
    const listAgents = result.toolExecutors.get('ListTimAgents');
    const sendAgentMessage = result.toolExecutors.get('SendTimAgentMessage');
    ...
    try {
      ...
      expect(listResult).toEqual({
        content: [{ type: 'text', text: JSON.stringify({ responseFor: 'ListTimAgents' }) }],
      });
      expect(sendResult).toEqual({
        content: [{ type: 'text', text: JSON.stringify({ responseFor: 'SendTimAgentMessage' }) }],
      });
    } finally {
      socket.destroy();
      await closeServer(parentServer);
