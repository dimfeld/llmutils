...
describe('Claude MCP parent agent-tool router', () => {
    ...
    expect(responses).toEqual([
      {
        ...
        requestId: 'forged-stop',
        success: false,
        error: 'Agent tool StopTimAgent is not allowed for this caller',
      },
      {
        type: 'agent_tool_response',
        requestId: 'forged-start',
        success: false,
        error: 'Agent tool StartTimAgent is not allowed for this caller',
      },
    ]);
  ...
  test.each([
    {
      ...
      forbiddenTools: ['FinishTimAgent'] as const,
    },
    {
      role: 'subagent' as const,
      allowedTools: CLAUDE_SUBAGENT_TOOL_NAMES,
      forbiddenTools: ['StartTimAgent', 'StopTimAgent'] as const,
    },
  ])(
    ...
    async ({ role, allowedTools, forbiddenTools }) => {
      ...
      const inputs: Record<ClaudeAgentToolName, unknown> = {
        StartTimAgent: { type: 'tester', executor: 'claude-code', initialMessage: 'start' },
        ListTimAgents: {},
        SendTimAgentMessage: { name: 'orchestrator', message: 'hello' },
      ...
      for (const toolName of forbiddenTools) {
        const method = {
          StartTimAgent: dispatcher.startAgent,
          StopTimAgent: dispatcher.stopAgent,
          FinishTimAgent: dispatcher.finishAgent,
        }[toolName];
  ...
  test.each([
    {
      role: 'orchestrator' as const,
      forbiddenTool: 'FinishTimAgent' as const,
    },
    {
      role: 'subagent' as const,
      forbiddenTool: 'StartTimAgent' as const,
    },
  ...
  test('returns manager errors without changing the permission path', async () => {
    const dispatcher = {
      ...
      listAgents: vi.fn(async () => ({ agents: [] })),
      sendAgentMessage: vi.fn(async () => {
        throw new Error('AgentManager rejected SendTimAgentMessage');
      }),
      stopAgent: vi.fn(),
      finishAgent: vi.fn(),
    };
    ...
    expect(responseById.get('manager-error')).toEqual({
      ...
      requestId: 'manager-error',
      success: false,
      error: 'AgentManager rejected SendTimAgentMessage',
    });
  ...
  test('denies an oversized permission request and keeps the bridge usable', async () => {
    const result = await setupPermissionsMcp({
      allowedTools: ['Read'],
      interactiveApprovalEnabled: true,
    });
    ...
    const client = await connectRawAndCollect(
      ...
      [
        `${oversized}\n`,
        `${JSON.stringify({
          requestId: 'permission-after-oversized',
          type: 'permission_request',
          tool_name: 'Read',
          input: {},
        })}\n`,
  ...
  test('makes cleanup idempotent and cancels an open requester once', async () => {
    ...
    const closeSpy = vi.spyOn(result.socketServer, 'close');
