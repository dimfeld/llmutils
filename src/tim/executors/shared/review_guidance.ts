...
export function buildBatchReviewRejectionGuidance(planId: string): string {
  return `   - When you reject one of your own batch-review findings, record it with \`tim review-issues reject ${planId} --content "<the finding>" --file <path> --line <line> --reason "..."\`. This review produces no reviewer output file, so use the explicit fields rather than \`--from-review\`. Recording it keeps the final full-plan review from re-raising a finding you already settled.`;
}
...
- Apply this severity gate to findings from ordinary reviews before starting another fix or review round. The shared calibrated reviewer rubric below is the sole definition of the four severity levels:
${indentBlock(REVIEW_SEVERITY_RUBRIC, '  ')}
- For ordinary-review findings within the ordinary iteration loop, findings with severity \`critical\` or \`major\` are **blocking** and must be fixed in-loop; findings with severity \`minor\` or \`info\` are **non-blocking** and must be rejected with a concrete reason or captured immediately as follow-up tasks through the bounded-handoff wording below. Non-blocking findings must NEVER by themselves trigger an implementer round or another review rerun. The reviewer's JSON output already carries a \`severity\` field per issue; for legacy free-text output, map \`CRITICAL\`/\`MAJOR\` to blocking and \`MINOR\` to non-blocking. When you perform a review yourself rather than running the reviewer command, assign each of your own findings one of the same four severities before applying this gate; use the shared rubric above as the definition.${structuralReviewGuidance}
