...
export function buildPrCreationPrompt(options: PrCreationPromptOptions): string {
  ...
  const prompt: string[] = [
    ...
    '',
    `1a. Check the working copy status: run \`${statusCommand}\``,
    '',
    `1b. Check the current commit and branch: run \`${
      options.vcsType === 'jj' ? 'jj log -r @ -n 1' : 'git log -n 1 --decorate'
    }\``,
    '',
    `1c. Get the current bookmark/branch name: run \`${branchNameCommand}\``,
    '',
    '   - Store this as `<branch-name>` for use in subsequent steps',
    `   - Base branch is \`${options.baseBranch}\` and comparison base is \`${baseRef}\``,
    '',
    `1d. Check the commit history on the branch: run \`${historyCommand}\``,
    '',
    '## Step 2: Examine Changes',
    '',
    `List the files that will be included in the PR: run \`${diffSummaryCommand}\``,
    '',
    ...
    '- Manual Testing Runbooks section copied from the Plan Context when the plan details contain "Manual Testing Runbooks"; omit this section when the plan details do not contain "Manual Testing Runbooks"; preserve the runbook titles, steps, and expected outcomes so reviewers can manually walk through the delivered feature',
