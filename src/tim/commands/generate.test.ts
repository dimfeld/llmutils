...
describe('handleGenerateCommand with --claude flag', () => {
  ...
  beforeEach(async () => {
    ...
    // Create temporary directory
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tim-generate-claude-test-'));
    tasksDir = path.join(tempDir, 'tasks');
    await fs.mkdir(tasksDir, { recursive: true });
  ...
  test('creates blocking plans via tim add and updates relationships end-to-end', async () => {
    ...
    mockExecutorExecute.mockImplementationOnce(async () => {
      ...
      if (!blockingPlan || blockingPlan.id === undefined) {
        throw new Error('Blocking plan was not created by tim add');
      }
...
describe('handleGenerateCommand direct_mode configuration logic', () => {
  ...
  const runStreamingPromptSpy = mock(async () => {
    ...
    const yamlContent = `# yaml-language-server: $schema=https://raw.githubusercontent.com/dimfeld/llmutils/main/schema/tim-plan-schema.json
id: test-plan-001
title: Test Plan
goal: Test goal
details: Test details
...
phases:
  ...
  const waitForEnterSpy = mock(
    async () => `# yaml-language-server: $schema=https://raw.githubusercontent.com/dimfeld/llmutils/main/schema/tim-plan-schema.json
id: test-plan-001
title: Test Plan
goal: Test goal
details: Test details
...
`
  ...
  beforeEach(async () => {
    ...
    // Create temporary directory
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tim-generate-directmode-test-'));
    tasksDir = path.join(tempDir, 'tasks');
    await fs.mkdir(tasksDir, { recursive: true });
    ...
    await moduleMocker.mock('../../tim/llm_utils/run_and_apply.js', () => ({
      runStreamingPrompt: runStreamingPromptSpy,
...
describe('handleGenerateCommand with --issue flag (Issue Tracker Abstraction)', () => {
  ...
  beforeEach(async () => {
    ...
    // Create temporary directory
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tim-generate-issue-test-'));
    tasksDir = path.join(tempDir, 'tasks');
    await fs.mkdir(tasksDir, { recursive: true });
...
describe('handleGenerateCommand claude_mode configuration logic', () => {
  ...
  beforeEach(async () => {
    ...
    // Create temporary directory
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tim-generate-claude-config-test-'));
    tasksDir = path.join(tempDir, 'tasks');
    await fs.mkdir(tasksDir, { recursive: true });
...
describe('handleGenerateCommand research and task prompt behavior', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tim-generate-research-'));
    tasksDir = path.join(tempDir, 'tasks');
    await fs.mkdir(tasksDir, { recursive: true });
  ...
  test('prompt includes task generation instructions using tim CLI', async () => {
    // Simulate executor adding tasks to the plan file
    mockExecutorExecute.mockImplementationOnce(async () => {
      const plan = await readPlanFile(planPath);
      plan.tasks = [{ title: 'Test Task', description: 'A test task', done: false }];
    ...
    // Should include tim tools update-plan-tasks CLI instruction
    expect(prompt).toContain('tim tools update-plan-tasks');
  });
