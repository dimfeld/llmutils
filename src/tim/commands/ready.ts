...
function displayListFormat(
  ...
  log('\n' + '─'.repeat(80));
  log(`\nRun ${chalk.bold('tim agent <id>')} to execute a plan`);
  log(`Run ${chalk.bold('tim show <id>')} to see full details`);
}
...
export async function handleReadyCommand(options: ReadyCommandOptions, command: any) {
  ...
  if (readyPlans.length === 0) {
    ...
    if (!options.all) {
      log(`Use ${chalk.bold('tim ready --all')} to include plans claimed in other workspaces.`);
    }

    if (!options.unassigned) {
      log(
        `Use ${chalk.bold('tim ready --unassigned')} to focus on unassigned plans that are ready.`
      );
    }
