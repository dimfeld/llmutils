...
describe('CollaborativeAgentSession root activation', () => {
  ...
  test('installs role-scoped root tools and launches mixed persistent providers through the manager', async () => {
    ...
    try {
      expect(session.claudeAgentToolContext.allowedTools).toEqual(
        new Set(['StartTimAgent', 'ListTimAgents', 'SendTimAgentMessage', 'StopTimAgent'])
      );
      expect(
        session.codexDynamicToolProvider.definitions.map((definition) => definition.name)
      ).toEqual(['StartTimAgent', 'ListTimAgents', 'SendTimAgentMessage', 'StopTimAgent']);
      expect(session.manager.listAgents().agents).toHaveLength(1);
      expect(session.orchestratorEnvironmentIdentity).toEqual({
        messagingDirectory: session.manager.sessionRuntime.runtime.rootPath,
        ...
        expect.objectContaining({
          ...
          claudeCodeOptions: expect.objectContaining({
            ...
            agentToolContext: expect.objectContaining({
              ...
              allowedTools: new Set(['ListTimAgents', 'SendTimAgentMessage', 'FinishTimAgent']),
            }),
          }),
          noninteractive: false,
          terminalInput: false,
          agentEnvironmentIdentity: expect.objectContaining({
            messagingDirectory: session.manager.sessionRuntime.runtime.rootPath,
            name: 'claude-worker',
            role: 'subagent',
            type: 'implementer',
        ...
        expect.objectContaining({
          ...
          dynamicToolProvider: expect.objectContaining({
            ...
            definitions: expect.arrayContaining([
              expect.objectContaining({ name: 'ListTimAgents' }),
              expect.objectContaining({ name: 'SendTimAgentMessage' }),
              expect.objectContaining({ name: 'FinishTimAgent' }),
            ]),
          }),
        })
      );
      expect(mocks.prepareSubagentExecution).toHaveBeenCalledWith(
        expect.objectContaining({
          agentType: 'reviewer',
          executor: 'codex-cli',
          promptContext: { mode: 'persistent-agent', agentName: 'codex-review' },
        })
      );
  ...
  test('uses the same provider-neutral root semantics for a Codex orchestrator', async () => {
    ...
    try {
      ...
      expect(rootToolResult.contentItems[0]?.text).toBe('{"agents":[]}');
    } finally {
      await session.close();
    }
  });
