...
function buildSimplifyPrompt(
  ...
  parts.push(
    ...
    'For test files, optimize for readability over DRY-ness: repetitive, self-contained tests that each set up fixtures and assert inline are healthy. Do not flag near-identical tests merely because they repeat setup, and do not push to consolidate tests into parameterized loops or "load everything" mega-helpers. Only raise test findings when the structure genuinely obscures what is being tested or hides error-prone differences between near-copies.\n',
    ...
    'Before finishing, run linting and tests for the project. Both must pass. If a simplification breaks either command, repair it or revert that specific fix.\n',
