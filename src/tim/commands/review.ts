...
} from '../subagents/remediation_plan.js';
...
export interface ReviewCommandOptions {
  ...
  /**
   * `--remediation-plan` / `--no-remediation-plan`. Undefined when neither flag was passed,
   * which leaves the decision to config and to whether an advisor is configured.
   */
...
const PLANLESS_REVIEW_REJECTED_OPTIONS: Array<{
  ...
  {
    key: 'remediationPlan',
    flag: '--remediation-plan',
    // Only the opt-in form is rejected. `--no-remediation-plan` asks for something a planless
    // review already does, so failing on it would be pointless.
    rejectWhen: (options) => options.remediationPlan === true,
  },
  {
  ...
  try {
    ...
    const executeReviewFlow = async (): Promise<void> => {
      ...
      try {
        ...
        // Remediation planning runs before formatting so the plan reaches every output
        // sink — stdout, --output-file, and the saved review history — as part of the
        // review result rather than as a second thing the caller has to go find.
        if (
          shouldGenerateRemediationPlan({
            config,
            cliOverride: options.remediationPlan,
            isFullScopeReview: !isScoped,
            issues: reviewResult.issues,
          })
        ) {
          reviewLog(chalk.cyan('Consulting the advisor for a remediation plan...'));
          try {
            const remediation = await withReviewLogger(() =>
              generateRemediationPlan({
                planId: contextPlanId,
                planTitle: planData.title ?? 'Untitled Plan',
                issues: reviewResult.issues,
                reviewScope: options.structuralOnly
                  ? 'the full plan, structural simplification pass'
                  : 'the full plan',
                recommendations: reviewResult.recommendations,
                actionItems: reviewResult.actionItems,
                configPath: globalOpts.config,
            ...
            if (remediation.remediationPlan) {
              reviewResult.remediationPlan = remediation.remediationPlan;
              reviewLog(
                chalk.cyan(`Remediation plan generated by the advisor (${remediation.executor}).`)
              );
            }
          } catch (remediationErr) {
            // A failed consultation must not discard a completed review.
            const remediationErrorMessage =
              remediationErr instanceof Error ? remediationErr.message : String(remediationErr);
            warn(
              chalk.yellow(
                `Warning: Could not generate an advisor remediation plan: ${remediationErrorMessage}`
              )
            );
          }
        }
        ...
        sendStructured({
          ...
          recommendations: reviewResult.recommendations,
          actionItems: reviewResult.actionItems,
          remediationPlan: reviewResult.remediationPlan,
        });
