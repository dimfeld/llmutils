...
describe('handlePromoteCommand', () => {
  ...
  test('sync-mode legacy promote fails before creating promoted child plans', async () => {
    const originalPlan: PlanSchema = {
      id: 1,
      title: 'Legacy Parent',
      goal: 'Promote with legacy data',
      details: '',
      status: 'pending',
      tasks: [
        {
    ...
    vi.mocked(loadEffectiveConfig).mockResolvedValue({
      ...
      sync: {
        role: 'main',
        nodeId: 'main-promote-legacy-test',
        allowedNodes: [],
      },
    } as never);

    await expect(handlePromoteCommand(['1.1'], { config: configPath })).rejects.toThrow(
      'legacy task rows without UUIDs'
    );

    const parent = (await resolvePlanByNumericId(1, tempDir)).plan;
    expect(parent.tasks?.map((task) => task.title)).toEqual(['Legacy task']);
    expect(parent.dependencies ?? []).toEqual([]);
    expect(getPlanByPlanId(db, context.projectId, 2)).toBeNull();
  });

  test('local legacy promote rolls back all direct writes when parent update fails', async () => {
    const originalPlan: PlanSchema = {
      id: 1,
      title: 'Legacy Parent',
      goal: 'Promote with legacy rollback',
      details: '',
      status: 'pending',
      tasks: [
        {
    ...
    db.prepare(
      `CREATE TRIGGER fail_legacy_promote_parent_tasks
       BEFORE DELETE ON plan_task
       WHEN OLD.plan_uuid = '${parentRow!.uuid}'
       BEGIN
         SELECT RAISE(FAIL, 'injected legacy promote failure');
       END`
    ).run();

    await expect(handlePromoteCommand(['1.1-2'], { config: configPath })).rejects.toThrow(
      'injected legacy promote failure'
    );
