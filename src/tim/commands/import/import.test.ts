...
describe('handleImportCommand', () => {
  ...
  test('requires an issue ID', async () => {
    await expect(handleImportCommand()).rejects.toThrow('Issue ID is required');
    expect(getIssueTracker).not.toHaveBeenCalled();
  });

  test('requires an issue ID even in clipboard mode', async () => {
    await expect(handleImportCommand(undefined, { clipboard: true })).rejects.toThrow(
  ...
  describe('Issue Tracker Abstraction', () => {
    ...
    test('requires an issue ID for each configured issue tracker', async () => {
      await expect(handleImportCommand()).rejects.toThrow('Issue ID is required');
      expect(getIssueTracker).not.toHaveBeenCalled();
    });
  });
