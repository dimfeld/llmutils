...
import { sanitizeErrorMessage } from './mailbox_helpers.js';
import {
...
export class MailboxConnection {
  ...
  private handleData(chunk: Buffer): void {
    ...
    this.frameReceived = true;
    // Bun can defer the peer FIN event until after the first next-turn
    // callback. The second turn lets handleEnd win before delivery when the
    // peer half-closes immediately after sending a complete frame.
    setImmediate(() => {
      setImmediate(() => {
