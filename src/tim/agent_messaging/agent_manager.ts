...
export class AgentManager {
  ...
  public listAgents(caller?: AgentCallerIdentity): ListAgentsResult {
    const excludedAgentId =
      caller === undefined ? undefined : this.directory.resolveCaller(caller).id;
    return this.directory.listAgents(excludedAgentId);
  }
      ...
      const prepared = await operation.awaitBoundary(
        this.options.agentPreparer.prepare({
          ...
          promptContext: {
            ...
            agentName: reservation.identity.name,
          },
        })
      );
  ...
  public async finishAgent(
    ...
    if (!parsed.success) {
      throw new AgentManagerError('invalid_request', 'FinishTimAgent request is invalid');
    }
  ...
  public async stopAgent(caller: AgentCallerIdentity, request: unknown): Promise<StopAgentResult> {
    ...
    if (!parsed.success) {
      ...
      throw new AgentManagerError('invalid_request', 'StopTimAgent request is invalid');
    }
  ...
  private validateSendRequest(request: unknown): {
    ...
    if (!result.success) {
      ...
      throw new AgentManagerError('invalid_request', 'SendTimAgentMessage request is invalid');
    }
    return result.data;
  }
