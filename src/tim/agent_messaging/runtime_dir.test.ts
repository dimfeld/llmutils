...
import {
  ...
  resolveAgentMessagingRuntimeParentDirectory,
...
describe('agent_messaging/runtime_dir', () => {
  ...
  test('uses the short system temporary path for macOS sockets', () => {
    const longMacTemporaryDirectory = '/var/folders/12/abcdefghijklmnopqrstuvwxyz1234567890/T';

    expect(resolveAgentMessagingRuntimeParentDirectory('darwin', longMacTemporaryDirectory)).toBe(
      '/tmp'
    );
    expect(resolveAgentMessagingRuntimeParentDirectory('linux', longMacTemporaryDirectory)).toBe(
