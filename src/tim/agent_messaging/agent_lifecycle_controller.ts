  ...
  public finish(message: string | undefined): FinishAgentResult {
    if (this.providerExit !== undefined || this.terminal.terminalClaimed) {
      throw new AgentManagerError(
        ...
        'FinishTimAgent is not available after the provider has exited'
      );
    }
    if (this.options.record.state === 'finishing') {
      if (!this.terminal.finishRequested) {
        throw new AgentManagerError(
          'finish_not_available',
          'FinishTimAgent was not the operation that started this finishing transition'
        );
      }
      this.saveFinishFallback(message);
      return Object.freeze({ state: 'finishing' });
    }
    if (this.options.record.state === 'stopping' && isGracefulStopPhase(this.phase)) {
      // StopTimAgent may leave the provider's current turn active while it asks
      // the agent for its final response. Accept a redundant FinishTimAgent from
      // that response and retain its message as the final-result fallback.
      // The graceful stop still owns the shutdown and terminal cause.
      this.saveFinishFallback(message);
      return Object.freeze({ state: 'finishing' });
    }
    if (this.options.record.state !== 'running-active') {
      throw new AgentManagerError(
        'finish_not_available',
        'FinishTimAgent is only available during an active provider turn'
      );
    }
