...
/** Select a short socket-safe parent while preserving the normal temp directory elsewhere. */
export function resolveAgentMessagingRuntimeParentDirectory(
  platform: NodeJS.Platform,
  temporaryDirectory: string
): string {
  return platform === 'darwin' ? '/tmp' : temporaryDirectory;
}
...
export class AgentMessagingRuntimeDirectory {
  ...
  public static async create(): Promise<AgentMessagingRuntimeDirectory> {
    ...
    try {
      const parentDirectory = resolveAgentMessagingRuntimeParentDirectory(
        process.platform,
        os.tmpdir()
      );
      rootPath = await fs.mkdtemp(path.join(parentDirectory, AGENT_MESSAGING_RUNTIME_PREFIX));
