...
describe('agent messaging transport integration', () => {
  ...
  test('handles errors from connections rejected at the receiver limit', async () => {
    const session = await createSession();
    const source = await register(session, orchestratorDraft('limit-error-root'), () => 'steered');
    const targetHandle = await register(
      session,
      subagentDraft('limit-error-target', 'limit-error-target'),
      () => 'steered',
      { maxConnections: 1 }
    );
    const occupyingSocket = net.createConnection(targetHandle.registration.socketPath);
    await new Promise<void>((resolve, reject) => {
      occupyingSocket.once('connect', resolve);
      occupyingSocket.once('error', reject);
    });

    const rejectedSocket = net.createConnection(targetHandle.registration.socketPath);
    rejectedSocket.on('error', () => undefined);
    await new Promise<void>((resolve) => {
      rejectedSocket.once('connect', () => {
        rejectedSocket.destroy(new Error('intentional rejected connection reset'));
        resolve();
      });
    });
    await new Promise<void>((resolve) => setImmediate(resolve));
    occupyingSocket.destroy();
    await new Promise<void>((resolve) => setImmediate(resolve));

    await expect(
      session.sendMessage(identity(source), target(targetHandle.registration.name), {
        content: 'receiver remains available',
      })
