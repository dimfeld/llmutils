...
class AgentStartOperation {
  ...
  public async awaitResource<T>(operation: Promise<T>, attach: (value: T) => void): Promise<T> {
    let attached = false;
  ...
  public async rollback(): Promise<void> {
    ...
    // Never wait for an unresolved registration or launch promise. The
    // awaitResource() continuation still attaches a resource that resolves
    // later, and attachMailbox()/attachHandle() starts bounded late cleanup.
    ...
    // Provider cleanup can outlive the failed StartTimAgent operation. Release
