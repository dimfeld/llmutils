...
slackDigestCommand
  ...
  .option('--refresh-concurrency <count>', 'Number of PRs to refresh at once (default: 2)')
  .action(async (options, command) => {
...
prCommand
  ...
  .action(async (pr, body) => {
    const { handlePrCommentCommand } = await import('./commands/pr.js');
    await handlePrCommentCommand(pr, body.join(' ')).catch(handleCommandError);
  });
...
workspaceCommand
  ...
  .option('--clone-method <method>', 'Clone method: git, cp, cow, or mac-cow (overrides config)')
  .option('--source-dir <path>', 'Source directory for cp/cow methods (overrides config)')
  .option('--repo-url <url>', 'Repository URL for git method (overrides config)')
  .option('--create-branch', 'Create a new branch (default when a plan ID is given)')
  .option('--no-create-branch', 'Do not create a new branch (default without a plan ID)')
...
// Register the subagent command with implementer, tester, tdd-tests, advisor, and reviewer subcommands
