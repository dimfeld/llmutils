...
export function findYamlStart(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m && Object.keys(timConfigSchema.shape).includes(m[2])) {
...
export async function extractMarkdownToYaml(
  ...
  try {
    ...
    await Bun.write('tim-parse-failure.yml', convertedYaml);
    error('Failed to parse YAML. Saved raw output to tim-parse-failure.yml');
    throw e;
  }
  ...
  try {
    ...
    if (!result.success) {
      ...
      await Bun.write('tim-validation-failure.yml', convertedYaml);
    ...
    // Save the failed YAML for debugging
    await Bun.write('tim-conversion-failure.yml', convertedYaml);
    error(
      'Failed to parse YAML output from LLM conversion. Saved raw output to tim-conversion-failure.yml'
    );
