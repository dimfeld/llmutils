...
describe('preparePhase with Claude option', () => {
  ...
  test('appends research findings to existing research section', async () => {
    const researchPlan: PlanSchema = {
      id: 6,
      title: 'Oneshot With Research',
      goal: 'Keep research history',
      details: [
        'Initial details',
        '',
        '## Research',
        '',
        '### 2024-03-15 09:30 UTC',
        '',
    ...
    planFilePath = path.join(tasksDir, 'research-history-plan.yaml');
    await fs.writeFile(planFilePath, yaml.stringify(researchPlan));

    invokeClaudeCodeForGenerationSpy.mockResolvedValueOnce({
      generationOutput: `tasks:\n  - title: "Initial Task"\n    description: "Updated description"\n    steps:\n      - prompt: "Generated step"\n        done: false`,
      researchOutput: 'Follow-up findings from Claude',
    });
    ...
    expect(updatedPlan.details).toContain('Earlier investigation notes');
    expect(updatedPlan.details).toContain('Follow-up findings from Claude');

    const researchHeadingCount = (updatedPlan.details.match(/## Research/g) || []).length;
    expect(researchHeadingCount).toBe(1);

    const entryCount = (updatedPlan.details.match(/### /g) || []).length;
