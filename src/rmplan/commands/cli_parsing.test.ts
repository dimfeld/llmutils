...
describe('CLI Parsing for --next-ready flag', () => {
  test('generate command should parse --next-ready flag correctly', () => {
    ...
    const generateCommand = program
      ...
      .option('--simple', 'For simpler tasks, generate a single-phase plan that already includes the prompts')
      .option('--autofind', 'Automatically find relevant files based on plan')
      .option('--quiet', 'Suppress informational output')
      .option('--no-extract', 'Do not automatically run the extract command after generating the prompt')
  ...
  test('prepare command should parse --next-ready flag correctly', () => {
    ...
    const prepareCommand = program
      ...
      .description('Generate detailed steps and prompts for a specific phase. Can be a file path or plan ID.')
  ...
  test('agent command should parse --next-ready flag correctly', () => {
    ...
    const agentCommand = program
      ...
      .option('--workspace <id>', 'ID for the task, used for workspace naming and tracking. If provided, a new workspace will be created.')
      .option('--auto-workspace', 'Automatically select an available workspace or create a new one')
      .option('--new-workspace', 'Allow creating a new workspace. When used with --workspace, creates a new workspace with the specified ID. When used with --auto-workspace, always creates a new workspace instead of reusing existing ones.')
      ...
      .option('--direct', 'Call LLM directly instead of copying prompt to clipboard during preparation')
