...
describe('executeBatchMode captureOutput integration', () => {
  ...
  test('surfaces parser failure in batch mode when output cannot be stringified', async () => {
    // Create a circular object to force JSON.stringify failure in generic parser path
    const circ: any = {};
    circ.self = circ;
    ...
    await executeBatchMode(
      {
        currentPlanFile: '/tmp/plan.yml',
        config: {} as any,
        executor: { execute: failingExecuteSpy, filePathPrefix: '' } as any,
        baseDir: '/tmp/repo',
        dryRun: false,
        executorName: 'codex-cli',
      },
