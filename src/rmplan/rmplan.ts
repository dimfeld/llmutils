...
program
  ...
  .action(async (planFile) => {
    ...
    try {
      ...
      const modelSpec = splitConfig.models?.convert_yaml || 'google/gemini-2.0-flash';
      const model = createModel(modelSpec);
      ...
      const outputDir = path.join(
        ...
        path.basename(resolvedPlanFile, path.extname(resolvedPlanFile)) + '-phases'
      );

      // Prepare options for saveMultiPhaseYaml
      const extractOptions: ExtractMarkdownToYamlOptions = {
        output: outputDir,
        // projectId will be generated by saveMultiPhaseYaml
        // issueUrl is not directly applicable here unless we add a new option
      };
