...
import type { AgentCommandSharedOptions } from './executors/types.js';
...
program
  ...
  .action(async (planFile, options) => {
    ...
    const executor = executors.get(selectedExecutorName);

    if (!executor) {
      error(`Executor "${selectedExecutorName}" not found.`);
      process.exit(1);
    }
    ...
    try {
      ...
      while (stepCount < maxSteps) {
        ...
        const stepPreparationResult = await prepareNextStep(config, planFile, {
          rmfilter: executor.contextConfig.runRmfilter,
          previous: true,
          selectSteps: false,
        ...
        if (executor.contextConfig.runRmfilter) {
        ...
        try {
          ...
          const baseApplyLlmEditsOptions = {
            interactive: true,
            baseDir: gitRoot,
            // copyRetryPrompt: false, // Default, can be overridden by executor if needed
            // originalPrompt: contextContent, // The executor receives contextContent, it can use it for originalPrompt
          };
