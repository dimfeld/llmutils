 ...
 */
export function parseExecutorOutput(_executorName: string, raw: unknown): ParsedExecutorOutput {
  ...
  const metadata =
    obj.metadata && typeof obj.metadata === 'object' ? (obj.metadata as any) : undefined;
  const steps = isSteps((obj as any).steps)
    ? ((obj as any).steps as Array<{ title: string; body: string }>)
    : undefined;
