...
export async function loadReviewHierarchy(
  ...
  const tasksDir = await resolveTasksDir(rmplanConfig);
...
export async function runExternalReviewForCodex(
  ...
  const reviewDoc = await loadRepositoryReviewDoc(options.gitRoot, options.rmplanConfig);
...
function applyTaskScope(
  ...
  const scopedTasks = planData.tasks.filter((task) =>
    normalizedTargets.has(normalizeTitle(task.title))
  );
