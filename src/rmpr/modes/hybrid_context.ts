...
function getLineCommenterForFile(
  ...
  switch (extension) {
    ...
    case '.sql':
      return (text: string) => `-- ${text}`;
    // Svelte is handled specially within insertAiCommentsAndPrepareDiffContexts due to context-dependent prefixing
...
/**
 * Inserts AI-prefixed comments with unique IDs into file content and prepares diff contexts.
 * Line numbers in comments are 1-based.
 */
export function insertAiCommentsAndPrepareDiffContexts(
  originalContent: string,
  commentsForFile: DetailedReviewComment[],
  filePath: string
): HybridInsertionResult {
  debugLog('insertAiCommentsAndPrepareDiffContexts', filePath);
  ...
  const errors: Array<{ comment: DetailedReviewComment; error: string }> = [];
  const commentDiffContexts: CommentDiffContext[] = [];
  ...
  for (const comment of sortedComments) {
    ...
    // Generate AI comments with the ID embedded
    const aiPrefixedBodyLines = commentBody.split('\n').map((line, index) => {
      if (index === 0) {
        // Include the ID in the first line
        return currentPrefixer(`AI (id: ${commentId}): ${line}`);
      } else {
        return currentPrefixer(`AI: ${line}`);
      }
    });
    ...
    if (isBlockComment) {
      ...
      fullAiComment = [startMarkerLine, ...aiPrefixedBodyLines, endMarkerLine].join('\n');
    } else {
      // Single-line comment: insert before the startLine
      addToMapList(insertBefore, startLine, aiPrefixedBodyLines);
      fullAiComment = aiPrefixedBodyLines.join('\n');
    }
  ...
  return {
    contentWithAiComments: newLines.join('\n'),
    commentDiffContexts,
    errors,
  };
}
