...
export async function runCleanupService(): Promise<CleanupServiceResult> {
  ...
  log(`  - Workspaces: ${result.workspaces.cleanedCount} cleaned, ${result.workspaces.failedCount} failed`);
...
export function scheduleCleanupService(intervalHours: number = 24): () => void {
  ...
  runCleanupService().catch(err => {
    error(`[Cleanup Service] Initial cleanup failed: ${err instanceof Error ? err.message : String(err)}`);
  });
  ...
  const intervalId = setInterval(() => {
    runCleanupService().catch(err => {
      error(`[Cleanup Service] Scheduled cleanup failed: ${err instanceof Error ? err.message : String(err)}`);
    });
  }, intervalMs);
