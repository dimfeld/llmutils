...
describe('Task Logs Cleanup', () => {
  ...
  afterEach(async () => {
    ...
    if (testLogIds.length > 0) {
      await db.delete(taskLogs).where(and(
        eq(taskLogs.taskId, testTaskId),
        gte(taskLogs.id, Math.min(...testLogIds))
      ));
    }
  ...
  test('getOldTaskLogsCount should count logs older than specified date', async () => {
    ...
    const results = await db.insert(taskLogs).values([
      {
        taskId: testTaskId,
        timestamp: now,
        logLevel: 'info',
  ...
  test('deleteOldTaskLogs should delete logs older than retention period', async () => {
    ...
    const remainingLogs = await db
      .select()
      .from(taskLogs)
      .where(eq(taskLogs.taskId, testTaskId));
