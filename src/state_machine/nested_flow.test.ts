...
describe('Nested Flow State Machine', () => {
  test('demonstrates simple nested state machine with event passing', async () => {
    ...
    class SimpleInitialNode extends InitialNode<SimpleStates, {}, SimpleEvent> {
      ...
      async post(result: null, store: SharedStore<{}, SimpleEvent>): Promise<StateResult<SimpleStates, SimpleEvent>> {
  ...
  test('demonstrates a state machine that processes, waits, and then continues', async () => {
    ...
    class InnerInitialNode extends InitialNode<InnerStates, {}, ProcessEvent> {
      ...
      async post(result: null, store: SharedStore<{}, ProcessEvent>): Promise<StateResult<InnerStates, ProcessEvent>> {
    ...
    class InnerProcessingNode extends Node<InnerStates, {}, ProcessEvent, ProcessEvent[], ProcessEvent[], null> {
      constructor() {
        super('inner_processing');
      }

      async prep(store: SharedStore<{}, ProcessEvent>): Promise<PrepResult<ProcessEvent, ProcessEvent[]>> {
    ...
    class InnerWaitingNode extends Node<InnerStates, {}, ProcessEvent, ProcessEvent[], ProcessEvent[], null> {
      constructor() {
        super('inner_waiting');
      }
    ...
    class OuterInitialNode extends InitialNode<OuterStates, {}, ProcessEvent> {
      ...
      async post(result: null, store: SharedStore<{}, ProcessEvent>): Promise<StateResult<OuterStates, ProcessEvent>> {
        return {
          status: 'transition',
          to: 'outer_process',
          actions: [{
    ...
    class ProcessFlowNode extends FlowNode<OuterStates, {}, ProcessEvent, ProcessEvent, ProcessEvent[]> {
    ...
    const waitingAction = initialResult.actions?.find(a => 
      a.type === 'OUTPUT' && a.payload.includes('waiting for input'));
    ...
    const innerResultAction = finalResult.actions?.find(a => 
      a.type === 'OUTPUT' && (a.payload === 'Inner process complete' || a.payload.includes('complete')));
    expect(innerResultAction).toBeDefined();
    
    // Should include the outer machine's final action
    const outerFinalAction = finalResult.actions?.find(a => 
      a.type === 'OUTPUT' && a.payload === 'Outer process complete');
