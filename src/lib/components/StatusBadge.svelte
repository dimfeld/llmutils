<script lang="ts">
  import type { PlanDisplayStatus } from '$lib/server/db_queries.js';

  let {
    status,
    label: labelOverride,
    colorClass: colorClassOverride,
  }: { status: PlanDisplayStatus; label?: string; colorClass?: string } = $props();

  const colorMap: Record<PlanDisplayStatus, string> = {
    in_progress: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
    ready: 'bg-teal-100 text-teal-800 dark:bg-teal-900/30 dark:text-teal-300',
    blocked: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300',
    pending: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300',
    queued: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300',
    needs_attention: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
    needs_review: 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300',
    review_deferred: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
    reviewed: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300',
    recently_done: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
    done: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
    cancelled: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
    deferred: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
  };

  const labelMap: Record<PlanDisplayStatus, string> = {
    in_progress: 'In Progress',
    ready: 'Ready',
    blocked: 'Blocked',
    pending: 'Pending',
    queued: 'Queued',
    needs_attention: 'Needs Attention',
    needs_review: 'Needs Review',
    review_deferred: 'Review deferred',
    reviewed: 'Reviewed',
    recently_done: 'Recently Done',
    done: 'Done',
    cancelled: 'Cancelled',
    deferred: 'Deferred',
  };

  let colorClass = $derived(
    colorClassOverride ??
      colorMap[status] ??
      'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'
  );
  let label = $derived(labelOverride ?? labelMap[status] ?? status);
</script>

<span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {colorClass}">
  {label}
</span>
