...
title: config to customize PR created by review command - Configuration Schema
  and Infrastructure
goal: Add PR creation configuration to the tim config schema with proper
  validation
id: 111
...
tasks:
  - title: Add prCreation schema to configSchema.ts
    ...
    description: >
      Add a new `prCreation` field to the `timConfigSchema` in
      `src/tim/configSchema.ts` with Zod validation for `draft` (boolean,
  ...
  - title: Update getDefaultConfig function
    ...
    description: >
      Modify the `getDefaultConfig` function in `src/tim/configSchema.ts` to
  ...
  - title: Add config schema tests
    ...
    description: >
      Create comprehensive tests in `src/tim/configSchema.test.ts` for the
...
Acceptance criteria:
- New `prCreation` config section is added to `timConfigSchema`
