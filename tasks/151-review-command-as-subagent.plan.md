...
tasks:
  - title: Add review default executor config + docs
    ...
    description: Extend `review.defaultExecutor` in `src/tim/configSchema.ts` to
      accept `claude-code`, `codex-cli`, or `both` (no zod defaults). Update
      `src/tim/configSchema.test.ts` coverage for the new field and adjust
      README/config documentation to describe the new option and behavior.
  - title: Extend tim review CLI options
    done: true
    description: Add `--print/-p`, `--task-index`, and `--task-title` options to
      `tim review` in `src/tim/tim.ts`, supporting repeated flags and
  ...
  - title: Add/update automated tests for review changes
    ...
    description: "Add/adjust tests covering: `--print` behavior, task filtering and
      ...
      `review.defaultExecutor`, `--review-executor` override in `tim agent`,
...
- Add a --print or -p argument to "tim review" for running noninteractively that will return all the data without the prompts 
...
- In Claude code orchestrator, replace review subagent with directions to run tim review for Claude orchestrator

## Research

### Opportunity Summary
The current `tim review` command produces a single-executor review using the shared reviewer prompt and JSON schema output, but it lacks: (1) a dedicated non-interactive print flag, (2) task-scoped review targeting, (3) a built-in parallel review path that merges Claude + Codex outputs, (4) a review-specific default executor in config, and (5) orchestrator guidance to drive `tim review` instead of a reviewer subagent. The reviewer prompt also needs specific language updates (critical severity for “implemented but wrong” behavior, and the literal word “ultrathink”).

### Parallel Subagent Reports (Focused Codebase Scans)

#### Subagent A: CLI + Command Wiring
- `src/tim/tim.ts` defines the `review` CLI with many options, but no `--print/-p`, no task targeting flags, and no executor mode for “both”. The CLI only passes `options` into `handleReviewCommand` (no preprocessing).
- `src/tim/commands/review.ts` is the main orchestration for `tim review`. It:
...
- `tim agent` (`src/tim/commands/agent/agent.ts`) currently uses a single executor for implementation/testing/review via `options.executor || config.defaultExecutor`. There is no review-specific executor override today.
- Codex executor already derives `newlyCompletedTitles` during an iteration in both `src/tim/executors/codex_cli/normal_mode.ts` and `src/tim/executors/codex_cli/simple_mode.ts` via `parseCompletedTasksFromImplementer`. This is the right signal for scoping external reviews to the current iteration.

#### Subagent B: Prompts + Orchestrator
- `src/tim/executors/claude_code/agent_prompts.ts` defines the reviewer prompt used by both the review command and Codex normal-mode reviewer. This is the right place to add:
  - “ultrathink” in the review prompt.
  - Explicit instruction that “implemented but doesn’t meet requirements” is a CRITICAL issue.
- `src/tim/executors/claude_code/orchestrator_prompt.ts` instructs orchestrators to use the reviewer subagent in a “Review Phase”, and lists `tim-reviewer` as an available agent. Requirement says this must be replaced with directions to run `tim review` for the Claude orchestrator. That implies:
  - Update the “Available Agents” section (remove reviewer agent and/or replace with `tim review` guidance).
  - Update the “Review Phase” steps to call `tim review` (likely with `--print`, and optional task filtering).
  - Update “Important Guidelines” (remove “DO NOT review code directly, use reviewer agent”) to match the new workflow.
 - Reviews also run inside the Codex executor (`src/tim/executors/codex_cli/normal_mode.ts`) via an internal reviewer step that uses `getReviewerPrompt`. Prompt changes must account for this path as well.

#### Subagent C: Review Output + Executors
- Review mode requires strict JSON output conforming to `src/tim/formatters/review_output_schema.ts`. `createReviewResult` in `src/tim/formatters/review_formatter.ts` always parses JSON and throws if invalid.
- Codex review-only mode (`src/tim/executors/codex_cli/review_mode.ts`) already supplies the JSON schema and returns output suitable for parsing.
...
- Command entry point: `src/tim/tim.ts` (review CLI options).
- Review orchestration: `src/tim/commands/review.ts`.
- Context gathering: `src/tim/utils/context_gathering.ts` (no task filtering today).
- Reviewer prompt template: `src/tim/executors/claude_code/agent_prompts.ts`.
- Orchestrator instructions: `src/tim/executors/claude_code/orchestrator_prompt.ts`.
- Review formatting/schema: `src/tim/formatters/review_formatter.ts`, `src/tim/formatters/review_output_schema.ts`.
- Config schema: `src/tim/configSchema.ts` and tests in `src/tim/configSchema.test.ts`.
- Review command tests: `src/tim/commands/review.test.ts`.
- Task selection helpers: `src/tim/utils/task_operations.ts` (find by title, interactive select).
...
- `tim review` supports a `--print/-p` mode that is fully non-interactive and prints the complete review output without any selection prompts.
...
- The Claude orchestrator no longer invokes a reviewer subagent; it runs `tim review` instead.
- When running reviews during `tim agent`, `review.defaultExecutor` is used for the review phase even if `--executor` specified a different executor for implementation. `--review-executor` overrides this.
- Codex executor’s internal review step is fully replaced by `tim review` output (including verdict and fix-loop decisions), not additive/advisory.
- When `tim agent` runs review during an iteration, it should scope the review to only the tasks completed in that iteration.
...
- Technical Plan & Risks: Introduce a review executor resolver and a merge pipeline that combines two JSON review outputs. Wire review executor selection into `tim agent` so review runs can use a different executor than implementation. Biggest risks are JSON parse failures and undefined behavior when one executor fails.
...
- Dependencies: `buildReviewPrompt`, `getReviewerPrompt`, `createReviewResult`, `parseJsonReviewOutput`, `generateReviewSummary`, `gatherPlanContext`, commander CLI in `tim.ts`.
...
1. **Add review executor config**
   ...
   - For `tim agent`, use `review.defaultExecutor` for the review phase regardless of `--executor` (implementation executor).
   - Add `--review-executor` to override `review.defaultExecutor` for `tim agent` review runs.
   - Update `configSchema.test.ts` and README/config docs.

2. **Add CLI flags for non-interactive print and task scoping**
   - Add `-p, --print` to `tim review` to suppress prompts and always emit full output in JSON format (override any other format settings).
   ...
   - Add `--review-executor <name>` to `tim agent`/`tim run` to override the review executor used during the review phase.
...
6. **Update Claude orchestrator workflow**
   - Replace the reviewer subagent directions in `orchestrator_prompt.ts` with steps to run `tim review` (likely `--print` and the chosen executor).
...
7. **Testing**
   - Add/update unit tests in `src/tim/commands/review.test.ts` for:
...
- Codex executor’s reviewer step feeds into its fix loop and verdict parsing; if review execution is redirected to `tim review` for agent workflows, verdict handling must be updated to interpret JSON-only outputs.
- The external review must fully satisfy Codex’s review-phase needs (verdict + fix loop), so the code must derive ACCEPTABLE/NEEDS_FIXES from the JSON review output produced by `tim review`.
...
2. Add CLI flags for `tim review`: `--print/-p` and task selection flags. Add `--review-executor` to `tim agent`/`tim run`. Update help text and option parsing.
...
7. Update `orchestrator_prompt.ts` to replace reviewer subagent guidance with `tim review` instructions.
8. Update `tim agent` to honor `review.defaultExecutor` (or CLI `--review-executor`) for review-phase execution, and ensure the chosen review executor is visible to the Claude orchestrator instructions.
...
- Run `tim review <plan> --print --format json --verbosity detailed` and confirm no prompts appear and output is complete JSON.
- Run `tim review <plan> --task-index 0 --print` (or title-based) and ensure prompt and output only reference the selected tasks.
- Run `tim review <plan> --executor both --print` and confirm merged output includes issues from both executors.
- Run `tim agent <plan> --executor codex-cli --review-executor claude-code` (or config `review.defaultExecutor`) and confirm review uses the override.
...
### Acceptance Criteria
- [ ] Functional: User can run `tim review` with `--print` and receive non-interactive output.
...
- [ ] Functional: `tim agent` uses `review.defaultExecutor` for the review phase and `--review-executor` can override it.
- [ ] Functional: Codex executor’s internal review step is fully replaced by `tim review --print`, and its verdict/fix loop is driven by the external review JSON.
...
- Updated reviewer prompt and Claude orchestrator guidance to use `tim review --print`.
