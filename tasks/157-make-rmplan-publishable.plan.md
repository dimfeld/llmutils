...
tasks:
  - title: Add version field to root package.json
    ...
    description: Add a version field to the root package.json to serve as the source
      of truth for the rmplan version. Start with version 0.1.0.
  ...
  - title: Update build.ts to generate dist/package.json
    ...
    description: "Modify build.ts to generate a dist/package.json during the build
      ...
      (@dimfeld/rmplan), bin entry, files array, repository info, license,
  ...
  - title: Add publish scripts to root package.json
    ...
    description: "Add scripts for publishing: publish:rmplan that runs build then
  ...
  - title: Update README with npm installation instructions
    ...
    description: Add a section to README.md explaining how to install via npm (npm
      install -g @dimfeld/rmplan or bun add -g @dimfeld/rmplan). Include note
  ...
  - title: Test publish workflow with npm pack
    ...
    description: "Run bun run build, then npm pack in dist directory to create a
      tarball. Install the tarball globally and verify rmplan command works
      correctly. Test key features: list, show, and Claude Code executor."
tags: []
---

For this we want to publish the built version of the rmplan.js script since it runs faster.

## Research

### Overview

This plan involves making `rmplan` publishable as an npm package so users can install it via `npm install -g rmplan` (or similar). Currently, installation requires cloning the repository and running `pnpm add -g file://$(pwd)`, which is cumbersome for end users.
...
1. **Build Script (`build.ts`)**: Located at the repository root, it uses Bun's bundler to create:
   - `dist/rmplan.js` (~8.9MB minified) - The main CLI bundle
...
2. **Package Name**: Options include:
   - `rmplan` (short, memorable, may be taken)
   - `@dimfeld/rmplan` (scoped, guaranteed available)
...
- **External Dependencies**: Verified that external dependencies (`effect`, `@valibot/to-json-schema`, `sury`) are NOT used in rmplan code, so no additional dependencies needed

### Decisions Made

Based on discussion with the user:

1. **Package Name**: `@dimfeld/rmplan` (scoped package)
...
```bash
# Install globally
npm install -g @dimfeld/rmplan
# Or with bun
bun add -g @dimfeld/rmplan
...
- [ ] Users can install via `npm install -g @dimfeld/rmplan`
- [ ] `rmplan` command works after installation
...
```json
{
  "scripts": {
    ...
    "publish:rmplan": "cd dist && npm publish"
  }
}
```
...
2. Install the tarball globally: `npm install -g ./rmplan-0.1.0.tgz`
...
2. Verify dist/ contains: `rmplan.js`, `claude_code/permissions_mcp.js`, all WASM files
3. Run `npm pack` in dist/ directory
4. Install the generated tarball: `bun add -g ./rmplan-*.tgz`
5. Verify `rmplan --version` works
6. Test `rmplan list`, `rmplan show --next`, and other commands
...
4. **Package Name Availability**: Check if `rmplan` is available on npm before choosing the name
