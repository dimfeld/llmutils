...
7. **Update plan list/display commands to use DB** — Update tim list, tim next, tim ready, and plan_display.ts to query from DB instead of scanning files. Most of these already have DB paths (loadPlansFromDb) — make DB the primary path and remove file-scanning fallbacks.
