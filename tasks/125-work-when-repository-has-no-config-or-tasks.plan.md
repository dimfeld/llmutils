...
tasks:
  ...
  - title: Update resolveTasksDir Function
    ...
    description: Modify `resolveTasksDir()` in `src/tim/configSchema.ts` to check
  ...
  - title: Update Claude Code Executor
    ...
    description: Modify `src/tim/executors/claude_code.ts` to add `--add-dir`
...
changedFiles:
  ...
  - src/tim/external_storage_utils.js
  - src/tim/external_storage_utils.ts
  - src/tim/path_resolver.js
  - src/tim/path_resolver.test.ts
  - src/tim/path_resolver.ts
  - src/tim/plans/mark_done.ts
  - src/tim/plans.ts
  - src/tim/repository_config_resolver.js
  - src/tim/repository_config_resolver.test.ts
  - src/tim/repository_config_resolver.ts
  - src/tim/resolvePlanFile.external.test.ts
  - src/tim/tim.ts
  - src/tim/storage/storage_manager.js
...
- Added targeted coverage via `src/tim/repository_config_resolver.test.ts`, expanded scenarios in `src/tim/configLoader.test.ts`, and new `resolveTasksDir` cases in `src/tim/configSchema.test.ts` to validate directory creation, metadata propagation, and path resolution for both local and external storage modes.
- Hardened `src/tim/resolvePlanFile.external.test.ts` so every filesystem operation first asserts the target lives inside the test's temporary home directory, preventing the suite from deleting a contributor's real `~/.config/rmfilter/repositories/...` data if the mocked homedir ever fails.
- Provided bridge modules (`src/common/git_url_parser.js`, `src/tim/repository_config_resolver.js`) alongside the earlier git URL parser work so runtime consumers and the CLI resolve the new implementations without a build step.
...
- Refactored plan operations (`tim add`, `generate`, `promote`, hierarchical imports, cleanup utilities, renumbering, and mark-done flows) to consume the shared helpers so plan files always land in the external repository directory when required.
...
- Added storage management utilities plus `tim storage list/clean` commands that surface repository metadata, plan counts, optional size summaries, and safe deletion workflows backed by targeted unit tests.
- Corrected `collectExternalStorageDirectories` to resolve config and tasks paths from persisted metadata (with sensible fallbacks) so `tim storage list/clean` honours repositories overriding `paths.tasks`, preventing real plan directories from being miscounted or deleted; added regression coverage in `src/tim/storage/storage_manager.test.ts` and `src/tim/commands/storage.test.ts` for metadata-driven overrides across list and clean workflows.
...
- Hardened git remote sanitisation by trimming credentials, query parameters, and fragments in `stripGitSuffix()` and `describeRemoteForLogging()` before deriving repository names or emitting external-storage notices, and added high-signal unit plus integration coverage (config loader logging, repository resolver naming, tim add with credentialed remotes) to prevent regressions.
- Enhanced external-storage messaging tests in `src/tim/configLoader.test.ts` to assert the new guidance, and documented the automatic fallback plus executor access in `README.md` for users working on third-party repositories.
...
If a directory being worked in does not have an rmfilter/tim config, then we should put all the configuration and task files under
...
When working with third-party projects where we cannot add configuration files directly to the repository, tim should automatically use an external storage location at `~/.config/rmfilter/repositories/{name}` derived from the git remote origin URL. This allows users to manage plans for any repository without modifying the repository itself.

### Expected Behavior/Outcome
- When no `.rmfilter/config/tim.yml` exists in a repository, tim automatically uses `~/.config/rmfilter/repositories/{repo-name}/` for configuration and task storage
- A message is displayed informing the user that external configuration storage is being used
- All tim commands work transparently whether using local or external storage
...
### Acceptance Criteria
- [ ] When no local config exists, tim uses `~/.config/rmfilter/repositories/{name}/` automatically
- [ ] Repository name is correctly derived from various git remote URL formats (HTTPS, SSH, different hosts)
- [ ] All tim commands function correctly with external storage
...
- [ ] External config can contain both rmfilter and tim configurations
...
### Summary
- The feature aims to enable tim to work with third-party repositories by storing configuration and task files in `~/.config/rmfilter/repositories/{name}` when no local config exists in the repository.
- Critical discovery: `resolveTasksDir()` in `src/tim/configSchema.ts` is the single function that determines where all plan files are stored, making it the primary integration point.
...
**tim Configuration Loading**

**File:** `/Users/dimfeld/Documents/projects/llmutils/src/tim/configLoader.ts`
...
- `findLocalConfigPath()`: Finds local override configs (`tim.local.yml`)

**Search Pattern:**
1. **Override Path**: If specified via CLI, uses that path exactly
2. **Default Path**: `{gitRoot}/.rmfilter/config/tim.yml`
3. **Local Override**: `tim.local.yml` in same directory as main config
...
**File:** `/Users/dimfeld/Documents/projects/llmutils/src/tim/configSchema.ts`
...
**Current Behavior:**
- All tim commands use `resolveTasksDir()` to determine where plan files are stored/searched
...
- Plans are stored in a directory determined by the `config.paths.tasks` setting in `.rmfilter/config/tim.yml`
...
**Key Functions:**
- `resolveTasksDir(config)` in `/Users/dimfeld/Documents/projects/llmutils/src/tim/configSchema.ts` (lines 317-327)
- `loadEffectiveConfig()` in `/Users/dimfeld/Documents/projects/llmutils/src/tim/configLoader.ts` manages configuration loading and merging
...
- Implemented in `readAllPlans()` in `/Users/dimfeld/Documents/projects/llmutils/src/tim/plans.ts` (lines 123-127)

**Naming Conventions:**
- Numeric ID-based naming: `{planId}-{slugified-title}.plan.md`
- Example: `125-work-when-repository-has-no-config-or-tasks.plan.md`
- Plan IDs are auto-generated using `generateNumericPlanId()` from `/Users/dimfeld/Documents/projects/llmutils/src/tim/id_utils.ts`
...
**Path Resolution:**
- `resolvePlanFile()` in `/Users/dimfeld/Documents/projects/llmutils/src/tim/plans.ts` (lines 166-247) handles:
...
- Defined in `/Users/dimfeld/Documents/projects/llmutils/src/tim/planSchema.ts`
...
- `cachedPlans` Map in `/Users/dimfeld/Documents/projects/llmutils/src/tim/plans.ts` (lines 34-41)
...
**Core Storage Layer**
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/plans.ts` - Primary plan file operations
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/configSchema.ts` - Configuration and path resolution
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/configLoader.ts` - Configuration loading and merging

**Supporting Utilities**  
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/id_utils.ts` - Plan ID generation and slugification
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/planSchema.ts` - Plan structure validation
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/planPropertiesUpdater.ts` - Plan property updates

**Workspace Management (Reference Pattern)**
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/workspace/workspace_manager.ts` - Shows abstracted workspace creation
- `/Users/dimfeld/Documents/projects/llmutils/src/tim/workspace/workspace_tracker.ts` - JSON-based tracking with default locations
...
- `src/tim/workspace/workspace_auto_selector.ts:64` - For workspace creation
- `src/tim/commands/workspace.ts:44` and `:418` - For workspace commands
...
**Common Options (`src/tim/executors/common.ts`):**
...
**Executor Factory (`src/tim/executors/factory.ts`):**
...
1. **Core Commands** (`src/tim/commands/`) - 89 files using `getGitRoot()`
...
- Should we support a `.timignore` file in the external config directory to exclude certain files from operations? Answer: No
