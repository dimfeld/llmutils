...
progressNotes:
  - timestamp: 2025-10-27T07:23:12.424Z
    text: Added optional discoveredFrom field to phase schema and implemented
      ...
      getDiscoveredPlans for use across tim.
    source: "implementer: Tasks 1-4"
  - timestamp: 2025-10-27T07:26:59.176Z
    text: Expanded tim test suite to cover discoveredFrom schema validation,
  ...
  - timestamp: 2025-10-27T07:32:21.151Z
    text: Extended tim validate command to detect orphaned discoveredFrom
...
tasks:
  - title: Add discoveredFrom field to plan schema
    ...
    description: Add the `discoveredFrom` field to `phaseSchema` in
      `src/tim/planSchema.ts` after the `parent` field. Use
  ...
  - title: Implement getBlockedPlans utility function
    ...
    description: Add `getBlockedPlans(planId, allPlans)` to `src/tim/plans.ts`.
  ...
  - title: Implement getChildPlans utility function
    ...
    description: Add `getChildPlans(planId, allPlans)` to `src/tim/plans.ts`.
  ...
  - title: Implement getDiscoveredPlans utility function
    ...
    description: Add `getDiscoveredPlans(planId, allPlans)` to
      `src/tim/plans.ts`. Returns all plans where `discoveredFrom ===
      planId`. Include JSDoc comments with @param and @returns annotations.
  - title: Add schema validation tests for discoveredFrom
    done: true
    description: "Create tests in `src/tim/plans.test.ts` that verify: (1) schema
  ...
  - title: Add discoveredFrom validation to validate command
    ...
    description: "Update `src/tim/commands/validate.ts` to: (1) check that
...
File: `src/tim/planSchema.ts` (around line 65, after `parent` field)
...
File: `src/tim/plans.ts` (add near other plan utility functions)
...
File: `src/tim/commands/validate.ts`
...
After implementation, the tim system will:
...
**Files Modified**:
- `src/tim/planSchema.ts`: Add `discoveredFrom` field (~5 lines)
- `src/tim/plans.ts`: Add 3 utility functions (~60 lines with docs)
- `src/tim/plans.test.ts`: Add comprehensive tests (~150 lines)
- `src/tim/commands/validate.ts`: Add `discoveredFrom` validation (~30 lines)
...
1. Add `discoveredFrom` field to `phaseSchema` in `src/tim/planSchema.ts`
...
1. Add all three functions to `src/tim/plans.ts` together
...
Implemented 'Add discoveredFrom field to plan schema', 'Implement getBlockedPlans utility function', 'Implement getChildPlans utility function', 'Implement getDiscoveredPlans utility function', 'Add schema validation tests for discoveredFrom', 'Add unit tests for getBlockedPlans', 'Add unit tests for getChildPlans', 'Add unit tests for getDiscoveredPlans', 'Add edge case tests for utility functions', 'Add integration tests for plan file operations', 'Add discoveredFrom validation to validate command', and 'Run full test suite and type checking'. Added an optional discoveredFrom field in src/tim/planSchema.ts to persist plan discovery lineage while maintaining backward compatibility. Implemented new inverse lookup helpers in src/tim/plans.ts that compute dependents, child plans, and discovered plans on demand from the plan map. Expanded src/tim/plans.test.ts with schema validation coverage, comprehensive unit tests for the new helpers (including cycles, missing references, and large data sets), and integration checks that ensure legacy files load while newly written plans round-trip the discoveredFrom field. Updated src/tim/commands/validate.ts to detect orphaned discovery references, optionally auto-remove them via a new fixDiscoveredFromReferences helper, and surface the results in command output and summary counters. Added matching tests in src/tim/commands/validate.test.ts that cover success, autofix, and --no-fix modes to guard the new behaviour. Verified the changes with bun run check, bun test, and bun run format.
