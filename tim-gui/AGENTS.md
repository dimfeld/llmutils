...
- **Prompt event wiring**: `TimGUIApp.swift` wsHandler matches `.structured(message: .promptRequest(...))` and `.structured(message: .promptAnswered(...))` tunnel messages, delegating to SessionState methods which handle replay safety internally. The backend also sends `prompt_cancelled` structured messages when a prompt times out without receiving an answer — tim-gui should handle this by clearing `pendingPrompt` (matching on `requestId`) to remove stale prompts from the UI.
...
- **MessageInputBar** (in `SessionsView.swift`): Auto-growing text field (1–5 lines) with Enter-to-send, Shift+Enter for newlines. Only visible when session is active. The scroll-to-bottom button is overlaid within the same bottom `safeAreaInset` container when active, avoiding a `GeometryReader`/preference feedback loop for input-bar height tracking.
...
- `ProjectTrackingModels.swift` — Domain types (`TrackedProject`, `TrackedWorkspace`, `TrackedPlan`, `PlanDisplayStatus`) and pure filter functions. `TrackedWorkspace.isRecentlyActive(now:)` encapsulates the 48-hour recency check (locked/primary/auto override regardless of date).
...
- **Use the group's classification key directly in grouped views**: When rendering items inside a grouped section (e.g., plans grouped by status), pass the group's key (e.g., `group.status`) to child views rather than recomputing it per item via a store method. The group key is correct by construction and avoids both redundant computation and potential inconsistency if the recomputation diverges.
