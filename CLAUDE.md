...
- Database layer: `db/` directory with SQLite-backed storage for assignments, workspaces, permissions, and project metadata
  ...
  - CRUD modules: `project.ts`, `assignment.ts`, `permission.ts`, `workspace.ts`, `workspace_lock.ts`, `plan.ts`, `pr_status.ts`, `webhook_log.ts`
...
- Executor system in `executors/` for different LLM integration approaches. The two main executors are claude_code and codex_cli. The others are not used much anymore. Both executors support a multi-phase review pipeline: `executeAnalysisPhase()` runs free-form analysis to produce a review guide (Claude returns `{ sessionId }` for resume, Codex returns void), and the standard `executeReviewMode()` runs structured JSON review. Claude also has `executeReviewModeWithResume()` which uses `--resume sessionId` to continue from the analysis phase with full context. Both executors always have `supportsSubagents = true`.
