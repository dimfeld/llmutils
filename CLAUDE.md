...
3. **Web interface** (`src/lib/`, `src/routes/`): SvelteKit-based plans browser and real-time sessions monitor (see `docs/web-interface.md` for conventions and gotchas)
   ...
   - Plan actions: `src/lib/remote/plan_actions.remote.ts` provides `startGenerate` and `startAgent` remote commands; `src/lib/server/plan_actions.ts` handles detached process spawning; `src/lib/server/launch_lock.ts` provides per-plan launch locks to prevent duplicate spawns before WebSocket session registration (cleared by `session:update` event or 30s timeout)
