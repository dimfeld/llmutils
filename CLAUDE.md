...
Use `bun run test` for the server/Node test project. Do not use `bun test` or `bunx vitest run` since it runs the wrong test runner.
...
- Workspace setup: `workspace_setup.ts` provides `setupWorkspace()`, a shared helper used by `agent`, `generate`, and `chat` commands. Encapsulates workspace selection (auto/manual/new), lock acquisition, plan materialization (accepts `planId` option, materializes from DB to `.tim/plans/{planId}.plan.md` instead of copying files), cleanup handler registration, and fallback-to-cwd behavior. Returns `WorkspaceSetupResult` with `branchCreatedDuringSetup` (true when a new branch was created locally, false when reusing an existing branch or not creating branches). When no plan ID is provided, branch checkout is skipped and the workspace's current branch is used as-is. For existing workspaces, also handles preparation (dirty check, branch checkout) and runs `workspaceUpdateCommands` from config. Branch setup operates entirely within the execution workspace — the primary workspace is not touched during setup. After switching, sends updated `session_info` to the headless adapter (via `updateHeadlessSessionInfo()`) so the web UI re-groups the session under the correct workspace. Stale local-only branches (exist locally but not on remote) are deleted and recreated from the base branch rather than reused — gated on `fetchSucceeded` to avoid data loss in offline mode
...
- Shared utilities captured in purpose-built modules:
  - `plan_display.ts`: Resolves plans (DB-first via `resolvePlanFromDb()`, returns nullable `planPath`) and assembles context summaries for both CLI output and MCP tooling. `resolvePlanFromDb()` throws `PlanNotFoundError` (defined in `plans.ts`); use `instanceof PlanNotFoundError` for error discrimination
...
3. **Web interface** (`src/lib/`, `src/routes/`): SvelteKit-based plans browser and real-time sessions monitor (see `docs/web-interface.md` for conventions and gotchas)
   ...
   - Cookie-based project persistence: `src/lib/stores/project.svelte.ts` has helpers (`setLastProjectId`, `getLastProjectId`, `projectUrl`) for remembering the last-selected project; cookie is httpOnly (server-read only). Also exports `getProjectAbbreviation()` and `getProjectColor()` for deterministic avatar generation from project display names, and `PROJECT_COLOR_PALETTE` for the color enum
   - UI state store: `src/lib/stores/ui_state.svelte.ts` manages transient UI preferences (plan pane collapse, message drafts per session, sidebar collapse). Session-scoped state keyed by connectionId, cleaned up on `session:dismissed`. Sidebar collapse persisted via cookie for SSR-safe hydration (read server-side in `+layout.server.ts`, written client-side via `document.cookie`). Initialized in root layout via Svelte context (`setUIState()`/`useUIState()`)
   ...
   - Active Work route: `/projects/[projectId]/active` — single-page scrollable dashboard with three sections: Needs Attention (plans needing input/review, actionable PRs), Running Now (active agent sessions), Ready to Start (ready plans with inline Run Agent button). Sections are collapsible, hidden when empty (see `docs/web-interface.md` for details)
   ...
   - Plan actions: `src/lib/remote/plan_actions.remote.ts` provides `startGenerate`, `startAgent`, `startChat`, `startRebase`, `startFinish`, and `finishPlanQuick` remote commands; `src/lib/server/plan_actions.ts` handles detached process spawning; `src/lib/server/launch_lock.ts` provides per-plan launch locks to prevent duplicate spawns before WebSocket session registration (cleared by `session:update` event or 30s timeout)
   ...
   - Project settings: `src/lib/remote/project_settings.remote.ts` provides `updateProjectSetting` command; validates setting names against a `settingValueSchemas` registry (featured, abbreviation, color, branchPrefix). Settings route at `/projects/[projectId]/settings` with featured toggle, abbreviation text input, color swatch picker, and branch prefix text input
   ...
   - Review thread actions: `src/lib/remote/review_thread_actions.remote.ts` provides `convertThreadToTask`, `resolveThread`, `replyToThread`, and `startFixThreads` commands for managing PR review threads from the web UI. `convertThreadToTask` uses `createTaskFromReviewThread` from `$tim/commands/review.ts` with duplicate detection via `[source:review-thread:{threadId}]` markers. `resolveThread` calls the GitHub GraphQL API and updates local `is_resolved` cache. `replyToThread` posts via GitHub API and inserts an optimistic local comment. `startFixThreads` spawns `tim pr fix` as a detached process following the `launchTimCommand()` pattern
   - Command bar search: `src/lib/remote/command_bar_search.remote.ts` provides a `searchCommandBar` query for the global command bar. Searches plans by title (LIKE) and exact planId match, PRs by title (LIKE) and exact pr_number match. Terminal-status plans only returned on exact planId match. DB query helpers in `src/lib/server/command_bar_queries.ts`. Component at `src/lib/components/CommandBar.svelte` with utility helpers in `src/lib/components/command_bar_utils.ts`
   - Dashboard data: `src/lib/remote/dashboard.remote.ts` provides `getActionablePrs` query for the Active Work dashboard — loads actionable PR data (own PRs with status changes, others' PRs needing review) from cached DB data with plan linkage. Classification logic in `src/lib/utils/pr_actionability.ts`
   - Plans browser helpers: `src/lib/server/plans_browser.ts` abstraction layer between route handlers and `db_queries.ts`; includes `getDashboardData()` for the Active Work dashboard (returns non-terminal plans without workspaces)
   - Dashboard utilities: `src/lib/utils/dashboard_attention.ts` provides pure derivation functions for the Active Work dashboard — `deriveAttentionItems()` (plan + PR attention items), `deriveRunningNowSessions()`, `deriveReadyToStartPlans()`. Defines `ActionablePr`, `PlanAttentionItem`, `PrAttentionItem` types
   - Shared utilities: `src/lib/utils/time.ts` provides `formatRelativeTime()` for human-readable relative timestamps; `src/lib/utils/keyboard_nav.ts` provides `isListNavEvent()`, `getAdjacentItem()`, and `scrollListItemIntoView()` for Alt+Arrow keyboard navigation across list views; `src/lib/utils/keyboard_shortcuts.ts` provides `isTypingTarget()` and `handleGlobalShortcuts()` for global keyboard shortcuts (Cmd+K/Ctrl+K command bar, Ctrl+/ search focus, Ctrl+1/2/3 tab navigation) wired into the root layout; `src/lib/utils/message_formatting.ts` provides client-side `getDisplayCategory()` and `formatStructuredMessage()` for computing display categories and formatting structured messages that were passed through from the server; `src/lib/utils/session_export.ts` provides `exportSessionAsMarkdown()`, `formatMessageAsMarkdown()`, `formatSessionHeader()`, and `generateExportFilename()` for session transcript export as markdown (used by SessionDetail copy/download buttons); `src/lib/utils/pr_update_events.ts` provides `hasRelevantPrUpdate()` and `shouldRefreshProjectPrs()` for client-side overlap detection when handling `pr:updated` SSE events
   ...
   - Rate limit monitoring: `src/lib/server/rate_limit_store.ts` provides `RateLimitStore` class with `Map<string, RateLimitEntry>` keyed by `'provider:label'`, extraction functions `extractClaudeRateLimit()` (from `LlmStatusMessage` with `rateLimitInfo` field) and `extractCodexRateLimit()` (from `TokenUsageMessage` with `rateLimits`), auto-pruning of expired entries, and `getWorstUsagePercent()` for icon color determination. Store lives on `SessionManager` instance. `handleStructuredSideEffects()` intercepts `llm_status` (Claude) and `token_usage` (Codex) messages, extracts rate limit data, and emits `rate-limit:updated` SSE event when data changes. Initial rate limit state included in SSE snapshot for new connections. Client types in `src/lib/types/session.ts` (`RateLimitEntry`, `RateLimitState`, `RateLimitUpdatedEvent`). `RateLimitIndicator.svelte` in header bar shows Gauge icon (hidden when no data, yellow at 80-90% usage, red at 90%+) with click popover showing per-provider per-window usage, reset countdown, and staleness. `LlmStatusMessage` in `src/logging/structured_messages.ts` extended with optional `rateLimitInfo` field carrying raw rate limit data from Claude Code's `rate_limit_event`
...
- When mirroring a skip predicate from a sibling code path (e.g. two blocks that must run/skip together), transcribe ALL conditions of the source predicate, even ones that look orthogonal to your new block. Partial mirroring breaks the invariant that the blocks stay in lockstep. Example: simplify's `shouldSkipSimplify` must mirror `shouldSkipFinalReview` in full, including the `finalReview === false` clause, even though simplify has its own `mode === 'never'` opt-out.
