...
- Workspace round-trip sync: `workspace_roundtrip.ts` manages pre/post-execution sync for workspace-aware commands. `prepareWorkspaceRoundTrip()` sets up the sync context. `runPreExecutionWorkspaceSync()` pulls the branch from origin (skipped for newly created branches) and captures `preExecutionState` for change detection. `runPostExecutionWorkspaceSync()` commits changes, compares against `preExecutionState`, and pushes to origin only if there are actual changes. If the branch was newly created during setup and no changes were made, the local branch is deleted and workspace metadata is cleared. All sync uses origin as intermediary — no direct workspace-to-workspace push
