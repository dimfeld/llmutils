...
- **A new short-circuit can silently disable existing coverage of a later guard**: When production code gains an early-return/retention rule that runs _before_ another protection path, existing tests may now hit the new short-circuit and stop exercising the path they were written for — while still passing. Whenever you add an earlier guard to a function with multiple protection branches, audit the existing tests for that function and re-seed their fixtures so each one still forces the specific branch under test (e.g. set `state: 'closed'` so an open-state retention rule doesn't intercept a test meant to exercise canonical-URL/`referencedPrUrls` reference matching in `cleanOrphanedPrStatus`).
...
- **Migration data-preservation tests that reset `schema_version` after `openDatabase` do not exercise a genuine pre-migration table shape.** `openDatabase()` migrates to the latest version before the test body runs, so the table already carries the new schema (e.g. the new CHECK constraint). Manually resetting `schema_version` to re-run a migration replays the SQL but against the already-migrated table — it proves rows survive a re-run, not that the new constraint behaves. Always pair such a test with a companion test that asserts the post-migration constraint actually accepts the new value (e.g. insert/update a plan with the newly-added status and confirm it persists).
