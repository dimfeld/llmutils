...
- **Slack notifier** (`src/lib/server/slack_notifier.ts`): Background review-request notification loop. Starts when at least one Slack workspace is configured, reads pending `pr_review_request` rows with `notified_at IS NULL`, groups them by PR, applies a fixed 30-second debounce, posts one Slack channel message per enabled repo/PR, and marks rows notified after a confirmed send. It is kicked by the webhook poller's `onPrUpdated` callback and also runs about every 15 seconds so debounced requests can fire without another webhook.
