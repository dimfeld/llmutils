...
- **Inline diffs**: Markdown review guide ` ```unified-diff ` fenced blocks are rendered as Pierre `Diff.svelte` instances. `MarkdownContent.svelte` uses per-filename `diffOverrides` to pass annotation and interaction props to each diff segment. **Virtualize Review Guide Diffs** in Project Settings controls lazy mounting for these blocks and is saved in browser `localStorage`.
...
- `RunningNowRow.svelte` — compact row with command type badge, plan title, workspace name, elapsed time. The badge shows the raw command name, except `ci-fix` which is shown as "Fix CI". Selects session before navigating to Sessions tab
...
- **`getPrStatus`** (`query`): Returns cached PR status for the plan from the DB. Response (`PrStatusResponse`): `{ prUrls: string[], invalidPrUrls: string[], prStatuses: PrStatusDetailWithRequiredChecks[], latestReviewGuidesByPrUrl: Record<string, LatestReviewGuideSummary>, tokenConfigured: boolean, configuredUsername: string | null }`. `prStatuses` includes the union of explicit PR URLs and auto-linked (webhook branch-matched) PRs from the `plan_pr` junction table, with nested review threads (`includeReviewThreads: true`), and each row is passed through `withRequiredCheckRollupStates()` so `check_rollup_state` reflects the required checks only. Non-URL entries and non-PR URLs from the plan's `pull_request` field are returned in `invalidPrUrls` rather than silently dropped. `tokenConfigured` gates the Full Refresh button visibility. `configuredUsername` is the resolved GitHub username (from `config.githubUsername` or the `gh` login), which the client needs to decide whether the viewer authored a PR — for example for the "Fix CI" gate.
...
- **`PrDetail.svelte`** — PR detail view with flat action buttons and expandable check, review, and review-thread sections. The Check Runs section shows a plain **"Fix CI"** button only when `canFixCi()` says that the viewer authored the PR and the effective (required-check) rollup is failing. The action starts a no-plan `tim pr fix-ci --pr ...` session through `startPrCiFix` and uses the same launch-state labels as the review-thread fixer.
...
- **Shared CI action eligibility** (`src/lib/utils/ci_fix_eligibility.ts`) — `canFixCi(pr)` is a pure helper used by both PR status components and the server launch path. It requires the viewer/author check (`isOwnPr`) and `checkRollupToSummaryStatus(pr.status.check_rollup_state) === 'failing'`. The gate uses the rollup because individual check-run rows can be missing after webhook ingestion. The server wrapper `isCiFixEligibleForUsername(db, prStatus, username)` (`src/lib/server/ci_fix_eligibility.ts`) first applies `withRequiredCheckRollupState()` and compares the author with `normalizeGitHubUsername()`, so both sides judge the same effective rollup. Clients get that same effective rollup from `getPrStatus` / the PR detail loader, and the author name from `configuredUsername`.
- **Plan-level Fix CI action** — `PrStatusSection.svelte` shows **"Fix CI"** for an eligible linked PR and starts the plan-scoped `tim pr fix-ci <planId>` flow through `startCiFix`. The button uses the same launch-state labels and disables itself while the plan's CI-fix session is starting or active.
- **`pr_fix_launch_state.ts`** — `getFixButtonState(input, defaultLabel = 'Fix Unresolved')` returns the shared button label and disabled state ("Starting...", "Fix Started", "Session Active", or the default label). The CI buttons pass `'Fix CI'` as the second argument; `FixButtonState['label']` is a closed union, so a new button label must be added there too.
...
- Each message becomes a `DisplayMessage`. Structured messages are passed through with `body: { type: 'structured', message: StructuredMessagePayload }` and `category: 'structured'`. The client computes display categories and formatting via `src/lib/utils/message_formatting.ts`. Non-structured TunnelMessages (log/error/warn/stdout/stderr) retain server-side formatting into text/monospaced body types with `category: 'log' | 'error'`. Messages carry the optional `agentName` from the tunnel or headless adapter, and `SessionMessage` renders it as the message source. stdout/stderr also carry an optional `origin` (`'lifecycle'`) propagated from the `writeStdout`/`writeStderr` logging calls (tagged by `LifecycleManager` in `src/tim/lifecycle.ts` and threaded through the tunnel protocol). `SessionDetail` hides `origin === 'lifecycle'` messages by default behind a header toggle; terminal/log-file rendering ignores `origin`.
...
- **Actions**: `sendPromptResponse(connectionId, requestId, value)`, `sendUserInput(connectionId, content)`, `dismissSession(connectionId)`, `endSession(connectionId)` — all call remote `command()` functions from `src/lib/remote/session_actions.remote.ts`.
...
- **Spawn handler** (`src/lib/server/plan_actions.ts`): `spawnTimProcess()` (internal) starts commands through the hidden `__daemon-launch`/`__daemon-monitor` stages. Each stage uses `Bun.spawn` with `{ detached: true }`; after the launcher exits, the monitor and worker have both a separate session/process group and a parent outside the web service's process tree. This double-fork-style boundary lets agents survive supervisor restarts that kill the web service's descendants, including PM2 restarts on macOS and Linux. stdout/stderr remain directed to a log file, and the monitor reports worker startup status through a short-lived status file so the web request retains the ~2000ms early-failure check. The executable defaults to `tim`; `TIM_PATH` in the web server environment can select a specific binary, and the project-level `timPath` config value takes precedence over that environment variable. The selected path is passed as `TIM_PATH` to the launched process so nested `tim` commands use it too. Public wrappers pass the appropriate CLI args. The spawned process starts an embedded WebSocket server and writes a session info file; the discovery client detects and connects to it, making it appear as a new session.
