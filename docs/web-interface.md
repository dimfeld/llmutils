...
- **`PrDetail.svelte`** — PR detail view with flat action buttons and expandable check, review, and review-thread sections. The Check Runs section shows a plain **"Fix CI"** button only when `canFixCi()` says that the viewer authored the PR and the stored check rollup is failing. The action starts a no-plan `tim pr fix-ci --pr ...` session and uses the same launch-state labels as the review-thread fixer.
...
- **Shared CI action eligibility** (`src/lib/utils/ci_fix_eligibility.ts`) — `canFixCi(pr)` is a pure helper used by both PR status components and the server launch path. It requires the normalized viewer/author check (`isOwnPr`) and `checkRollupToSummaryStatus(pr.status.check_rollup_state) === 'failing'`. The gate uses the rollup because individual check-run rows can be missing after webhook ingestion.
