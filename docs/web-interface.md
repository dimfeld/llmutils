...
- Non-tunnel `agent_session_end` structured messages set `triggersNotification` when the session's `session_info.interactive` flag is true. The client uses this for browser notifications and the blue attention dot, so new interactive commands should set `interactive: true` in headless session metadata rather than adding command names to a web UI allowlist.
