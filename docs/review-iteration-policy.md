...
through `StartTimAgent` or `SendTimAgentMessage`, not through subagent CLI flags. Each
...
After `StartTimAgent` returns, use its canonical name for later messages. If the
agent was already started, obtain the canonical name from `ListTimAgents`. Keep
...
`StartTimAgent` or `SendTimAgentMessage` assignments. This delegation difference is
...
- The selected-task batch review stays with the orchestrator. In an enabled
  session an advisory `StartTimAgent` reviewer may supply read-only findings, but
...
- An enabled orchestrator may never replace the formal review with a
  `StartTimAgent` reviewer. The formal process runs with fresh context and no
  collaborative tools.
