...
- `dailyDigest` (optional) - schedule for the [daily PR digest](#daily-pr-digest):
  - `enabled` - set to `true` to opt this workspace into daily digest runs. Missing or `false` disables daily digests for repos targeting this workspace.
...
Toggles the [daily PR digest](#daily-pr-digest) for the current repo. `enable` requires Slack to already be enabled for the repo (a workspace and channel must be set) and errors clearly otherwise. The digest posts to the same channel as review-request notifications when the target workspace also has `dailyDigest.enabled: true`.
...
Runs the daily digest immediately for every workspace with `dailyDigest.enabled: true` and digest-enabled repos. `--dry-run` computes and prints the digest sections for each repo without posting to Slack (and without requiring a usable Slack token), so you can preview what the scheduled run would send. Without `--dry-run`, it posts to Slack just like the scheduled run. Manual runs ignore the configured `weekdays`; weekdays only constrain the web server's scheduled timer.

### Update the Latest Daily Digest
...
`--refresh` fetches current status and digest history for the repo's cached open PRs from GitHub before computing the update. It also works with `--dry-run`; the preview does not call Slack, but the GitHub refresh updates the local database. If any PR refresh fails, the update stops before calling Slack.
...
Separate from the event-driven review-request notifier, tim can post a once-per-day digest of PRs that are stuck. It requires both workspace opt-in (`dailyDigest.enabled: true`) and per-repo opt-in (`tim slack digest enable`), then posts one message per digest-enabled repo to that repo's configured channel. The Slack `channel` and `ts` returned by `chat.postMessage` are stored per workspace, channel, repo, and local digest date so refreshes can update the latest stored message in place. When a new daily digest post is created, tim pins the new post and unpins the latest previous digest post for that repo/channel; updates keep the existing pin unless `--pin` is requested.

The digest has four PR sections:

- **Approved, not yet merged** - open, non-draft PRs whose review decision is `APPROVED`. Entries include cached PR size when available (`files changed` and `+/-` counts) and how long ago the latest known approval was submitted when local review data has that timestamp.
- **Awaiting review** - open, non-draft PRs that are not already approved and have an assigned individual reviewer who has not reviewed since being requested. Each entry lists cached PR size when available, the waiting reviewer(s), and how long they've waited. PR entry links point at `linear.review/{owner}/{repo}/pull/{number}`. The footer includes both the GitHub "View all PRs awaiting your review" search link and a Linear reviews link. When `dailyDigest.reviewGroups` is configured (see [config](#config)), this section is split into prioritized `*Awaiting review — {name} ({label})*` subsections by PR label, with unmatched PRs in a trailing default group.
- **Awaiting Review Response > 24 hours** - open, non-draft PRs where the latest review from a non-bot reviewer (other than the PR author, excluding `DISMISSED` reviews) was submitted more than 24 hours ago, and every active review request predates that review. These are PRs waiting on the _author_ to respond rather than on a reviewer, and are excluded from the approved and awaiting-review sections. Bots are identified by the conventional `name[bot]` login suffix. Each entry lists cached PR size when available, the reviewer who last reviewed, what they did (requested changes / commented / approved), and how long ago.
- **Other PRs ready for review for > 3 days** - open, non-draft PRs with a recorded `ready_at` timestamp older than three days that were not already shown in the approved, awaiting-review, or awaiting-review-response sections. Each entry lists cached PR size when available, how long the PR has been ready, and the time since the previous non-dismissed review, or notes that there has been no previous review.

If all sections are empty for a repo or channel, no new message is sent. If a stored digest message already exists, update runs can still replace it with an empty digest so merged or otherwise-cleared PRs disappear from the Slack post.
...
`tim slack digest` runs the digest immediately for all workspaces with `dailyDigest.enabled: true`; `tim slack digest --dry-run` previews the computed buckets without posting (and without requiring a usable token). See [Run the Daily Digest Now](#run-the-daily-digest-now).
...
# Optional: after the workspace has dailyDigest.enabled: true, opt this repo into
# the daily PR digest, then preview it
tim slack digest enable
tim slack digest --dry-run
```

With the web server running and webhook polling configured, review requests in enabled repos use these settings and mappings automatically. Digest-enabled repos in workspaces with `dailyDigest.enabled: true` additionally receive the daily digest at the workspace's configured `time`/`timezone`.
