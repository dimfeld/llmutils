...
**File layout**: All materialized plans live at `{repoRoot}/.tim/plans/{planId}.plan.md`. Every materialized plan is an editable working copy. `ensureMaterializeDir()` creates the directory and writes a `.gitignore` with `*.plan.md` to prevent accidental commits.

**Shadow copies**: Each materialized plan has an identical hidden shadow copy at `.tim/plans/.{planId}.plan.md.shadow`. The shadow records the exact state written during materialization and is used during sync to detect which fields the user actually changed (vs. round-tripped unchanged). Shadow files use the same YAML+markdown format as plan files and are parsed by `readShadowPlanFile()` — a side-effect-free parser that avoids the UUID auto-generation behavior of `readPlanFile()`.

**Core functions**:

- `materializePlan(planId, repoRoot, options?)`: Queries plan from DB, converts via `planRowToSchemaInput()`, and writes both the plan file and shadow copy using `generatePlanFileContent()` (a single serialization pass). Returns the file path.
- `materializeRelatedPlans(planId, repoRoot, options?)`: Materializes parent, children, siblings, and dependency plans as editable `.plan.md` files with shadows. It preserves an existing materialized file so local edits are never overwritten.
- `syncMaterializedPlan(planId, repoRoot, options?)`: Syncs any materialized plan. It (1) reads the shadow file if present, (2) reads the current file, (3) uses `diffPlanFields()` to detect changed fields — skips DB sync if nothing changed, (4) if changes are detected, reads current DB state and calls `mergePlanWithShadow()` to overlay only changed fields from the file onto the DB state, and (5) re-materializes the plan, updating both the file and shadow. When shadow is missing or corrupt, it falls back to full-overwrite behavior. When `force: true`, it bypasses merge and syncs the file plan directly. Options include `skipRematerialize` to avoid double re-materialization when called from `withPlanAutoSync()`.
...
For a plain synced scalar field (a timestamp or flag that references nothing),
use the shorter checklist in `docs/sync-operations-guide.md` ("Add A New Synced
Plan Scalar") instead. The checklist below is for reference columns only.
...
**Event handlers** (`src/common/github/webhook_event_handlers.ts`): Handlers that parse webhook payloads and apply incremental updates to PR status tables:
...
- `handlePullRequestReviewThreadEvent(db, payload, options)`: Handles both `pull_request_review_thread` and `pull_request_review_comment` deliveries. Applies thread resolution state and queues API refresh targets for the thread.
- `handleIssueCommentEvent(db, payload, options)`: Conversation (non-inline) comments on pull requests. It makes **no DB writes** and always returns `updated: false`; its only output is an inbox signal. It returns nothing for unparsable payloads, unknown repos, any `action` other than `created`, and plain issue comments (payloads with no `issue.pull_request` marker). Unlike the review and check-run handlers, it does **not** require an existing `pr_status` row — everything it needs comes from the payload, so comments on brand-new PRs are not lost.

All handlers return `HandlerResult` with `prUrls` (affected), `updated` flag, and `deferredApiCalls` (promises for async API fetches).

**Inbox signals**: handlers also return `inboxSignals?: InboxSignal[]` — raw, side-effect-free facts that the inbox producer turns into `inbox_item` rows. An `InboxSignal` is `{ kind, repo, prNumber, prUrl, prTitle?, prAuthor?, actor, actorType?, summary?, eventAt }`, plus `requestedReviewer` on the `review_requested` variant. `summary` holds a comment excerpt (first 200 characters, newlines collapsed — shared `getCommentExcerpt()`), a failing check name, or a dequeue reason. `webhook_log` payloads are pruned after 7 days, so each signal must carry every fact the inbox needs; nothing can be re-read from the payload later.

`InboxSignalKind` is the `inbox_item` kind list **minus** `reviewed_pr_comment`: at signal level every comment shape emits one `pr_comment` kind, and the producer reclassifies using `prAuthor` and review state. Which handler emits what:

| Signal                | Emitted by                           | Condition                                                                                                                           |
| --------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| `pr_comment`          | `handleIssueCommentEvent`            | `action: created` on a PR conversation comment                                                                                      |
| `pr_comment`          | `handlePullRequestReviewThreadEvent` | comment `created` with a body and an author                                                                                         |
| `pr_comment`          | `handlePullRequestReviewEvent`       | submitted review with state `COMMENTED` or `CHANGES_REQUESTED` **and** a non-empty body                                             |
| `pr_approved`         | `handlePullRequestReviewEvent`       | submitted review with state `APPROVED`                                                                                              |
| `review_requested`    | `handlePullRequestEvent`             | `action: review_requested`; `actor` is the sender (display), `requestedReviewer` is the relevance target                            |
| `pr_merged`           | `handlePullRequestEvent`             | `action: closed` with `merged_at` set                                                                                               |
| `merge_queue_removed` | `handlePullRequestEvent`             | `action: dequeued`; `summary` is the payload `reason`                                                                               |
| `ci_failure`          | `handleCheckRunEvent`                | `status: completed`, `classifyCheckRun()` is `failing`, and the check name is in `getRequiredCheckNames()` for the PR's base branch |

Notes on the edges: changes-requested is not its own kind, it is a comment. `ci_failure` is filtered to required checks **in the handler** — when a branch has no merge requirements, nothing is emitted and the producer does not re-filter. `merge_queue_removed` and `pr_merged` are both emitted faithfully when a dequeue is followed by a merge; suppressing the dequeue in that case is the producer's job. The review and check-run handlers still drop events when the `pr_status` row does not exist yet, so signals derived from them inherit that limitation.

`eventAt` is best-effort: the handler uses the payload timestamp (comment `created_at`, review `submitted_at`, `merged_at`, check-run `completed_at`, PR `updated_at`), falling back to the delivery's `receivedAt` passed in `WebhookHandlerOptions.receivedAt`, then to now.
...
**Ingestion orchestrator** (`src/common/github/webhook_ingest.ts`): `ingestWebhookEvents(db)` is the main entry point for webhook-based PR status updates. Flow: read cursor → fetch events from webhook server → insert into `webhook_log` (dedup by `delivery_id`) → dispatch to event handlers by `eventType` → collect and run deferred API calls via `Promise.allSettled` → advance cursor → prune old log entries. Dispatch uses the `WEBHOOK_EVENT_HANDLERS` map (`pull_request`, `issue_comment`, `pull_request_review`, `pull_request_review_thread`, `pull_request_review_comment`, `check_run`); an event type with no entry is skipped. Because duplicate deliveries never reach a handler, signals and side effects are produced only for freshly inserted rows. Returns `IngestResult` with `eventsIngested` (newly processed only), `prsUpdated`, `prsReadyForReview`, `reviewsSubmitted`, `inboxSignals`, and `errors` (including missing `WEBHOOK_INTERNAL_API_TOKEN`, per-event parse/handler failures, and deferred API call failures with `owner/repo#number` context for debugging). Returns early with empty result if `TIM_WEBHOOK_SERVER_URL` is not set. All callers (web remote commands, CLI) inspect `IngestResult.errors` and surface non-empty errors as warnings via `formatWebhookIngestErrors()` shared helper.

**Inbox-aware wrapper** (`src/lib/server/webhook_ingest_orchestrator.ts`): `ingestWebhookEventsWithInbox(db, options)` calls `ingestWebhookEvents(db)` and then feeds `inboxSignals` to the inbox producer. **Every production caller uses this wrapper, not `ingestWebhookEvents` directly** — the poller, both web PR refresh paths, and the CLI. Signals are ephemeral: a caller that advances the cursor without processing them consumes them permanently, so a direct `ingestWebhookEvents` call in a new refresh path is a silent data-loss bug. Ingestion errors still reject, but inbox processing runs inside its own try/catch after the cursor has advanced — a producer or session-manager failure is logged and cannot turn a completed ingest into a refresh failure. `options.sessionManager` (or the lazy `options.getSessionManager`) supplies the SSE emitter; the CLI passes neither.

`inboxSignals` collects the `InboxSignal[]` returned by the handlers into a per-run map keyed by `kind:prUrl:actor:eventAt`, so duplicate deliveries collapse while distinct comments from the same actor survive. A signal with no timestamp is backfilled with the delivery's `receivedAt`. Unlike `reviewsSubmitted`, inbox signals do **not** apply `isWebhookSideEffectAllowed()` at collection time: signals are side-effect-free raw facts, and the replay cutoff plus relevance and user filtering happen in the inbox producer that consumes them.
...
**Source**: rows come from the `inboxSignals` array on `IngestResult`, which webhook event handlers fill in during ingestion. See [Inbox signals](#webhook-log) above for the signal shape and for which handler emits which kind.
...
1. `owner/repo` → `getProject(db, constructGitHubRepositoryId(...))`; unknown repos are skipped.
2. Per-project context, resolved once per batch and cached by project ID: `getPrimaryWorkspacePath()` → `loadEffectiveConfig(undefined, { cwd })` → `getGitHubUsername({ githubUsername })`. Missing workspace, `inbox.prs.enabled === false`, or an unresolvable username skips every signal for that project.
3. `isWebhookSideEffectAllowed(config, signal.eventAt)` — the replay guard. Without it, a restart after downtime floods the inbox with old events. Row-level `dedupe_key` protection is not a substitute: the two guards cover different failure modes.
4. `tryCanonicalizePrUrl(signal.prUrl)`; a signal whose URL does not canonicalize is skipped rather than given a guessed key.
5. `merge_queue_removed` is dropped when a `pr_merged` signal for the same canonical URL is in the same batch, or when the `pr_status` row is already merged. A dequeue followed by a merge is not something to notify about.
6. Relevance, using `getPrStatusByProjectAndNumber()` (loaded once and reused for both author and reviewer checks) and falling back to `signal.prAuthor`: `review_requested` needs `requestedReviewer` to be the user; `pr_approved` / `pr_merged` / `ci_failure` / `merge_queue_removed` need the user to be the PR author; a `pr_comment` becomes `pr_comment` when the user authored the PR, or `reviewed_pr_comment` when they did not but the user has submitted a non-`PENDING` review. Everything else is dropped.
7. Actor filters on comment and approval kinds: `isBotLogin(actor)`, `actorType === 'Bot'`, or a normalized match in `inbox.prs.ignoreUsers`. Self-actions are skipped for every kind except `review_requested` (where `actor` is the sender, not the relevance target) and `pr_merged` (where the state change is still useful when the user performed the merge).
8. `upsertInboxItem()` with `dedupe_key = <kind>:<canonical url>` and self-contained display fields.

**Reviewer predicate** (`src/common/github/pr_relevance.ts`): `getReviewerPredicate(requestedReviewers, reviewRequests, reviews, username)` is pure and returns `{ isRequestedReviewer, hasSubmittedReview }` separately, so callers can use either signal. A `pr_review_request` row counts only when `removed_at IS NULL`, a review counts only when its state is not `PENDING`, and **every comparison normalizes both sides** with `normalizeGitHubUsername` — an un-normalized reviewer comparison was the latent case-sensitivity bug this helper was extracted to fix. `parseRequestedReviewers()` parses the `pr_status.requested_reviewers` JSON column, and `getReviewerPredicateForPr()` is the DB-reading wrapper. `partitionCachedProjectPrs` in `src/lib/remote/project_prs.remote.ts` uses the same helper.

**Live updates**: when the batch wrote at least one row and a session manager was supplied, the producer emits one `inbox:updated` SSE event carrying the affected project IDs — guarded by `hasInboxUpdateListeners()`. See [Push-Based Inbox Updates via SSE](web-interface.md#push-based-inbox-updates-via-sse).

**Pruning**: the producer calls `pruneOldInboxItems(db, 30)` at most once per 24 hours from the tick that processed signals, throttled by a module-level `lastPrunedAt` (the pattern from `shouldPruneOldEvents()` in `src/webhooks/db.ts`). `resetInboxPruneThrottle()` exists for test isolation and HMR. Prune failures are logged and do not fail the batch.
