...
**`tim set-task-done` / `tim add-task`**: Both use `resolvePlanFromDb()` from `plans.ts` to resolve plans by numeric ID or UUID. Writes go through `writePlanFile()` (DB-first, then optional file materialization).
