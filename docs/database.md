...
7. Actor filters on comment and approval kinds: `isBotLogin(actor)`, `actorType === 'Bot'`, or a normalized match in `inbox.prs.ignoreUsers`. Self-actions are skipped for every kind except `review_requested` (where `actor` is the sender, not the relevance target).
