...
6. Relevance, using `getPrStatusByProjectAndNumber()` (loaded once and reused for both author and reviewer checks) and falling back to `signal.prAuthor`: `review_requested` needs `requestedReviewer` to be the user; `pr_approved` / `pr_merged` / `ci_failure` / `merge_queue_removed` need the user to be the PR author; a `pr_comment` becomes `pr_comment` when the user authored the PR, or `reviewed_pr_comment` when they did not but the reviewer predicate matches. Everything else is dropped.
