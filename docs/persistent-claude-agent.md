...
1. **Launch readiness** — the subprocess and its stream-json writer exist, and
   the initial prompt write has settled successfully. The handle is returned
   here; `runClaudeSubprocess()` does not await later process exit.
...
public `ListTimAgents` schema. The manager maps provider facts onto its own
...
- `requestCloseAfterCurrentTurn()` backs FinishTimAgent. It sets a flag; stdin
