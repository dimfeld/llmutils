...
4. **Checks readiness criteria** for each candidate:
   ...
   - For `pending` plans: all dependencies must have a work-complete status (`done`, `needs_review`, `reviewed`, or `cancelled`)
...
- **Dependencies**: All dependency plans must have a work-complete status (`done`, `needs_review`, `reviewed`, or `cancelled`)
