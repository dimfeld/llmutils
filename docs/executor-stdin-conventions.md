...
- FinishAgent marks the current turn to close after its settled result. The MCP call and final assistant response must finish before stdin closes. Graceful StopAgent sends one shutdown instruction through the same writer and closes after that turn. Force stop uses the captured safe session-process capability for the exact child; it never accepts an arbitrary PID or display label.
...
`formatJsonMessage()` maps `background_tasks_changed` to a single lifecycle fact:

- A non-empty `tasks` array means background work is running.
- An empty `tasks` array means all background work has drained.

The older `task_started`, `task_notification`, `task_progress`, and `task_updated` messages remain available for console display but do not drive lifecycle state. `ScheduleWakeup` is treated like any other tool call and has no keep-alive behavior.
...
3. **Neither**: Non-interactive prompt lifecycle (`sendInitialPrompt` + result-time stdin close decision). The initial prompt does not end stdin; the result callback closes immediately for normal turns or keeps stdin open while background tasks are pending.
...
- **Auto-claim should happen before executor execution** (after workspace setup), so the plan is assigned even if the executor fails or is interrupted. This matches the ordering in `agent`, `generate`, and `chat` commands.
