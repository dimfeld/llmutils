...
proofGeneration:
  ...
  executor: claude-code
  model: opus
  artifactsDir: .tim/proofs # directory the executor writes into; cleared before each run
  instructions: |
...
Reruns are idempotent: prior proof artifacts (marked with a `tim-proof:` prefix) are soft-deleted before the new run begins, and the artifacts directory itself is cleared so leftover files from a previous run are not re-attached. If the executor errors mid-run, whatever files it has already written are still attached and the failure is surfaced to the caller. Files exceeding the 100 MB artifact size cap are skipped with a warning. The default `.tim/proofs` directory is added to the tim-managed `.tim/.gitignore`; if you point `artifactsDir` somewhere else, add that path to your own `.gitignore`.
