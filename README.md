...
- **Plan browsing**: Dedicated Plans tab with full status filter chips (pending, in-progress, blocked, recently done, done, cancelled, deferred), reset/all controls, search filtering by title/goal, and sortable plan list (by plan number, priority, recently updated, or status) for any project. Selecting a plan shows an expanded detail panel with full plan information including title, goal, status, priority, dependencies, assigned workspace, and timestamps. Data auto-refreshes while either tab is active.
