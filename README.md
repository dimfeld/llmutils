...
Webhook events also feed the PR inbox. An item is created when something needs your attention on a PR: a review request for you, a comment or approval on a PR you authored, a comment on a PR where you submitted a review, a required check failing on your PR, your PR being merged, or your PR leaving the merge queue without merging. Your own actions are ignored, except that your own PR merge events still create inbox items. Comment/review events from bot accounts are ignored, and events before `githubWebhooks.ignoreSideEffectsBefore` never create items. Repeated events on the same PR update one item instead of stacking up. Items need `githubUsername` (or a logged-in `gh`) to know which PRs are yours.
...
- **Inbox** - a global toolbar bell for recent PR notifications across projects, with an unread count, popup links, and read controls; plus a full inbox page (per-project or all-project) with per-kind action buttons that launch review-guide, PR-fix, or CI-fix sessions directly from notifications
...
tim autoreview --branch feature/my-branch --base main # Interactive review/fix loop in a managed workspace
...
tim review-guide diffview 123                        # Export the stored guide as diffview JSON
...
When the target has a resolvable GitHub PR — a plan-backed target whose plan links a PR, an explicit `--pr` target, or a `--current`/`--branch` target whose head branch matches an open PR — the orchestrator prompt gains a **PR Review Trail** section so the in-terminal review-and-fix loop is mirrored onto the PR as a durable, auditable record. For each issue you act on, the agent opens a review thread anchored at the referenced diff location (un-anchorable issues go in the review body), immediately replies-and-resolves issues you choose to ignore (with your stated reasoning), and after the fixes are committed replies to and resolves the threads it addressed. The agent is encouraged to perform these GitHub mechanics (`gh api` reviews, replies, and `resolveReviewThread`) in a subagent and to degrade gracefully — continuing the local loop and reporting the trail step as skipped — when `gh`/`GITHUB_TOKEN` auth is unavailable. Targets with no resolvable PR keep the original prompt with no PR Review Trail section.
...
tim tracks the latest review-request message per PR (pruning entries older than two weeks) and, when a non-bot user submits a review on that PR, adds an emoji reaction to the message: ✅ approved, 💬 commented, 🔄 changes requested. This needs the bot token to have `reactions:write`; reactions are best-effort and failures are only logged. If a review payload doesn't say whether the author is a bot, only reviewers with a Slack user mapping trigger reactions.

### Daily PR Digest

Separately, workspaces can opt into a once-per-day digest with `dailyDigest.enabled: true`, and repos in those workspaces can opt in with `tim slack digest enable`. The digest summarizes approved-but-unmerged PRs, including how long ago the latest known approval was submitted, non-approved PRs awaiting review, PRs awaiting an author response to a non-bot review left more than 24 hours ago, and other ready-for-review PRs older than three days, posting one message per repo to its configured channel and skipping repos with nothing to report. New daily digest posts are pinned and the latest previous digest pin for that repo/channel is removed; same-day refreshes update the existing pinned message in place. The schedule (`time`/`timezone`/`weekdays`) is per workspace, with `weekdays` defaulting to Monday through Friday; it requires webhook polling so the local PR data stays fresh, never uses `@`-mentions, and runs on a per-workspace timer in the web server. Run it on demand with `tim slack digest`, or preview without sending via `tim slack digest --dry-run`.
...
dialog shows Claude Code and Codex CLI with their default models.
...
`agentMessaging` is the flag for direct messaging between the orchestrator and its subagents. The contracts, configuration, session storage, Unix-socket mailbox transport, manager lifecycle, Claude MCP tool bridge, and persistent Claude provider runtime exist. No command activates this workflow yet, so a run with the flag on currently behaves exactly like a run with it off. See [docs/agent-messaging.md](docs/agent-messaging.md), [docs/agent-manager.md](docs/agent-manager.md), [docs/claude-mcp-bridge.md](docs/claude-mcp-bridge.md), and [docs/executor-stdin-conventions.md](docs/executor-stdin-conventions.md).
