...
- **Active work tracking**: Browse projects, workspaces, and active plans (in-progress/blocked) from `tim.db` in the Active Work tab
- **Plan browsing**: Dedicated Plans tab with full status filter chips (pending, in-progress, blocked, recently done, done, cancelled, deferred), reset/all controls, search filtering by title/goal, and plans grouped by display status into collapsible sections (In Progress, Pending, Blocked, Recently Done, Done, Deferred, Cancelled). Each group header shows the status icon, label, plan count, and an animated chevron toggle. Within each group, plans are sortable (by plan number, priority, or recently updated). Selecting a plan shows an expanded detail panel with full plan information including title, goal, status, priority, dependencies, assigned workspace, and timestamps. Data auto-refreshes while either tab is active.
