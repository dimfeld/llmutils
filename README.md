...
The token value supports `${ENV_VAR}` expansion at read time. The Slack app bot token needs `chat:write` access for the target channel, and the bot must be invited to that channel. Daily digest pin/unpin behavior also needs `pins:write`; digest posts still send if pinning fails, but tim logs a warning. A workspace may also carry an optional `dailyDigest: { time, timezone, weekdays, linearMilestones }` schedule for the daily PR digest (see below).
