...
Web UI: the plan detail page shows an **Artifacts** section beneath review issues with mime-type icons, inline thumbnails for images, filename, message, size, transfer state, and Delete/Restore actions. A drag-drop uploader and file picker post to `POST /api/artifacts`; downloads stream via `GET /api/artifacts/[artifactUuid]`. Proof artifacts open with `?view=1` so screenshots, videos, markdown, text, and code-like files render directly in a new browser tab when possible. A toggle reveals soft-deleted artifacts.
